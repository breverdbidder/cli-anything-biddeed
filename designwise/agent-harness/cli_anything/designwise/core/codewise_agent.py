...
Version: 1.2.0 (S2.2 — Real Stitch→Next.js converter)
...
- S2.2: convert_screen(), convert_all_screens(), create_feature_branch() real implementation
"""
...
class CodeWiseAgent:
    ...
    # ------------------------------------------------------------------
    # S2.2 canonical methods
    # ------------------------------------------------------------------

    async def convert_screen(
        self,
        screen_name: str,
        html_css: str,
        design_md_path: str = "DESIGN.md",
        ...
        """
        Full Stitch HTML → Next.js conversion for a single screen.

        Pipeline:
        1. Parse HTML from Stitch output
        2. Extract + map Tailwind classes to DESIGN.md CSS variables
        3. Replace hardcoded hex values → CSS variable references
        4. Substitute shadcn/ui components where applicable
        5. Wrap in Next.js page component (.tsx) with TypeScript types
        6. Return { tsx_code, route, component_name, imports }
        """
        # Derive route from screen_name
        route_map = {
            "landing-hero": "/",
            "landing": "/",
            "heatmap": "/heatmap",
        ...
        route = route_map.get(screen_name, f"/{screen_name}")

        # Load DESIGN.md brand tokens for enriched color mapping
        extra_tokens: dict[str, str] = {}
        try:
            from cli_anything.designwise.utils.brand_tokens import BrandTokenParser
            parser = BrandTokenParser(design_md_path)
            tokens = parser.parse()
            for color_name, hex_val in (tokens.get("colors") or {}).items():
                if hex_val not in COLOR_VAR_MAP:
                    extra_tokens[hex_val] = f"var(--color-{color_name.lower().replace(' ', '-')})"
        except Exception:
            pass

        combined_map = {**COLOR_VAR_MAP, **extra_tokens}

        # Replace hex colors in html_css
        html = html_css
        for hex_val, css_var in combined_map.items():
            html = html.replace(hex_val, css_var)
            html = html.replace(hex_val.upper(), css_var)
            html = html.replace(hex_val.lower(), css_var)

        # Replace class= → className=
        html = re.sub(r"\bclass=", "className=", html)

        # Convert style strings to React objects
        html = re.sub(r'style="([^"]*)"', _style_to_object, html)

        # Shadcn/ui substitutions (common mappings)
        SHADCN_MAP = {
            "<button": '<Button',
            "</button>": '</Button>',
            "<input": '<Input',
        ...
        for html_tag, shadcn_tag in SHADCN_MAP.items():
            html = html.replace(html_tag, shadcn_tag)

        component_name = "".join(w.capitalize() for w in screen_name.split("-"))
        imports = [
...
/**
 * {component_name} — ZoneWise.AI
 * Route: {route}
 * Generated by CodeWise Agent 04 (S2.2)
 * Source: Stitch 2.0 / @google/stitch-sdk
...
'''
    ...
    async def convert_all_screens(
        self,
        screens_dict: dict[str, str],
    ) -> dict[str, Any]:
        """
        Convert all Stitch screens to Next.js components.

        Args:
            screens_dict: {screen_name: html_css} from StitchWise batch output

        Returns:
            { files: [{path, content}], routes: {screen_name: route} }
        """
        files: list[dict[str, str]] = []
        routes: dict[str, str] = {}
        errors: list[str] = []

        for screen_name, html_css in screens_dict.items():
            try:
                result = await self.convert_screen(screen_name, html_css)
                route = result["route"]
                component_name = result["component_name"]
                tsx_code = result["tsx_code"]

                # Next.js App Router path: app/<route>/page.tsx
                if route == "/":
                    next_path = "app/page.tsx"
                else:
                    next_path = f"app{route}/page.tsx"

                files.append({"path": next_path, "content": tsx_code})
                routes[screen_name] = route
            except Exception as exc:
                errors.append(f"{screen_name}: {exc}")

        # Generate barrel index
        index_exports = "\n".join(
            f'export {{ default as {s.replace("-", "")} }} from "./{s}";'
            for s in screens_dict
        )
        files.append({"path": "components/designwise/index.ts", "content": index_exports})

        return {"files": files, "routes": routes, "errors": errors, "count": len(files)}
    ...
    async def create_feature_branch(
        ...
        """
        Create a git feature branch, write component files, validate, commit, and open a PR.

        Steps:
        1. git checkout -b feat/designwise-{screen_name}
        2. Write files to correct Next.js paths
        3. Run ESLint + TypeScript checks (non-blocking — reports but continues)
        4. git add + commit
        5. Create PR via GitHub API (feat → lab branch)
        """
        import subprocess as sp
        branch_name = f"feat/designwise-{screen_name}"
        work_dir = repo_path or self.repo_path

        def run(cmd: list[str]) -> tuple[int, str, str]:
            r = sp.run(cmd, cwd=work_dir, capture_output=True, text=True)
            return r.returncode, r.stdout.strip(), r.stderr.strip()

        # 1. Create branch
        rc, _, err = run(["git", "checkout", "-b", branch_name])
        if rc != 0:
            # Branch may already exist — try checkout
            rc2, _, err2 = run(["git", "checkout", branch_name])
            if rc2 != 0:
                return {"status": "error", "error": f"git checkout failed: {err2}", "branch": branch_name}

        # 2. Write files
        written: list[str] = []
        if files:
            for f in files:
                dest = Path(work_dir) / f["path"]
                dest.parent.mkdir(parents=True, exist_ok=True)
                dest.write_text(f["content"])
                written.append(f["path"])

        # 3. ESLint + TypeScript (best-effort)
        lint_results: dict[str, Any] = {}
        eslint_rc, eslint_out, _ = run(["npx", "eslint", "--ext", ".tsx,.ts", "app/", "--max-warnings=0"])
        lint_results["eslint"] = {"passed": eslint_rc == 0, "output": eslint_out[:500]}
        tsc_rc, tsc_out, _ = run(["npx", "tsc", "--noEmit"])
        lint_results["tsc"] = {"passed": tsc_rc == 0, "output": tsc_out[:500]}

        # 4. git add + commit
        run(["git", "add"] + (written if written else ["."]))
        commit_msg = f"feat(codewise): {screen_name} → Next.js (DesignWise S2.2)"
        rc, _, err = run(["git", "commit", "-m", commit_msg])
        if rc != 0:
            return {"status": "error", "error": f"git commit failed: {err}", "branch": branch_name}

        # 5. Create PR
        pr_result = await create_github_pr(
            branch_name=branch_name,
            title=f"feat(designwise): {screen_name} screen → Next.js",
            body=(
                f"## CodeWise: {screen_name}\n\n"
                f"- Branch: `{branch_name}`\n"
                f"- Files written: {', '.join(written) if written else 'none'}\n"
                f"- ESLint: {'✅' if lint_results['eslint']['passed'] else '⚠️'}\n"
                f"- TypeScript: {'✅' if lint_results['tsc']['passed'] else '⚠️'}\n\n"
                "_Auto-generated by CodeWise Agent 04 (S2.2)._"
            ),
        )
