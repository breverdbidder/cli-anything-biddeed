...
## Continuation (2026-07-04, 4th pass, same dispatch_id — duplicate dispatch, self-inflicted regression caught + corrected, two new findings)

Re-entered under the same `dispatch_id`/`chat_session` as every pass above. **This is a duplicate dispatch of the prior continuation (`66822de3`)** — same 5 counties, same brief. Ran a fresh native Workflow: `harvest -> parity-refresh -> adversarial-verify` pipelined per county (15 agents, 78 tool calls, 720K tokens), plus direct main-session SQL for root-cause work and the correction below.

### Self-inflicted regression, caught mid-session, fully corrected (report this transparently)

This pass's workflow ran the shared `refresh_parity_tier1_outcomes(county)` against all 5 counties as part of its fix stage. That function's wipe-first design (nulls `parity_status`/`parity_source` on every closed-status row before attempting to rematch via `tax_deed_outcomes`/`foreclosure_outcomes` only) collaterally destroyed matches on **santa_rosa** and **walton** that are actually backed by `public.realforeclose_aids` — a separate tier1 source the function doesn't know about, already vetted and shipped by an earlier same-shard continuation (`a11ab113`, `20260704_shard9_run2820_walton_santarosa_realforeclose_aids_cd.sql`).

The workflow's own adversarial-verify stage caught santa_rosa's drop (100.0%→51.7%) but **misdiagnosed it**: it had only read the *original* ghost-success revert report (`bf5cd982`, the top of this file) and concluded the pre-session 100.0% was itself the fabrication being correctly exposed. Acting on that, this session's main loop **restored the wipe** (re-nulled all 58 santa_rosa rows) — compounding the error. Before finalizing, a second read of the migrations directory surfaced `a11ab113`, which supersedes `bf5cd982`: it independently verified `realforeclose_aids` as real (82 santa_rosa rows with organic multi-batch `first_seen_at`/`source_run_id` values, a sampled `case_clerk_url` resolving live to a real Santa Rosa Clerk document viewer page) and had already restored santa_rosa on that basis, plus added 5 new walton matches. This session re-applied that exact, idempotent UPDATE pattern to both counties, undoing its own damage. Documented in full in `supabase/migrations/20260704_shard9_run2820_3rd_continuation_selfcorrection.sql` and logged (with the wrong-then-corrected audit trail preserved, not deleted) to `gold_standard_ultraloop_audit`.

**Net DB effect of this session: zero drift.** Final verified state matches `66822de3`'s already-established baseline exactly: santa_rosa C/D=100.0% (58/58), walton C/D=30.0% (9/30, the same real scrape-freshness sawtooth `66822de3` already explained — not re-investigated further this pass). Holmes (7.7%), sumter (unchanged, all FAIL except A/G/H), and osceola (5/10, A now genuinely PASS via real `osceola_clerk_civilmortgageforeclosures_pdf` rows spot-checked this pass — real FL court case numbers spanning 2011-2025, real `courts.osceolaclerk.com` source_url, not the deleted synthetic fixture family) all reproduce `66822de3`'s numbers exactly.

**Lesson recorded for future sessions** (in the migration file): before running `refresh_parity_tier1_outcomes` against any county, check `SELECT count(*) FROM realforeclose_aids WHERE county_slug = '<county>'` first — if non-zero, re-apply the `realforeclose_aids` UPDATE pattern in the same session immediately after, before evaluating or reporting the letter.

### Two new findings this pass (neither remediated — out of surgical scope, documented for follow-up)

1. **`fl_parcels.co_no` mislabeling is not isolated to Union County (co_no=63, per `SHARD-14 run3025`).** Independently confirmed a second instance: `fl_parcels` rows tagged `co_no=49` (osceola's official FL DOR code per `fl_counties`) are actually Liberty County data (`phy_city` = Bristol/Hosford/Telogia — 0 of osceola's 129 real parcel_ids match `co_no=49`). Osceola's actual parcel data lives under `co_no=59` in that table, but even joining on the correct co_no only covers 12/129 (9.3%) of osceola's parcels, and none of those 12 have `centroid_lat` populated — so `fl_parcels` cannot fix osceola's Letter I (card-complete) gap (129/134 rows have 0% lat/lng and 0% assessed/market value) even setting the mislabeling aside. This is a real, systemic, cross-county data-integrity bug in a shared statewide table — flagging again for a dedicated cross-shard session, consistent with the Union precedent's "outside surgical scope" call.
2. **Walton's `zone_standards` rows (backing its currently-PASSing G letter) have `source_url IS NULL` and `confidence_score IS NULL`** — no ordinance citation at all, unlike holmes/osceola's zone_standards (same generic R-1/R-2/C-1/A-1/I-1 codes, but at least `confidence_score=0.70` and a real `library.municode.com/fl/{county}` source_url). This matches the exact pattern the canon bans ("guessed standards = ghost-success"). Not fixed this pass (G isn't a targeted failing letter for this shard and zone_standards is a shared table) — flagging for whoever owns the zoning ingestion pipeline to check whether Walton's G=100% PASS should currently be trusted.

### Live external verification attempted (new this pass, genuinely zero-yield, honestly reported)

Ran the existing, already-hardened `scripts/county_outcome_harvester.py` live against all 5 counties' real RealForeclose/RealTaxDeed sites (`osceola.realforeclose.com`, `holmes.realforeclose.com`, `sumter.realforeclose.com`, `santarosa.realforeclose.com`, `walton.realforeclose.com`), 8 months back, unauthenticated (no `REALFORECLOSE_EMAIL`/`PASSWORD` available in this sandbox). All 5 hosts were reachable (no Cloudflare/522 block), but every single monthly `AUCTIONTYPE=F` preview query returned 0 `AITEM_` blocks — an honest zero-yield, not fabrication (confirmed by reading the script's source first; its self-referential ghost-success paths were already disabled by an earlier session dated today). Root cause is most likely the dispatch brief's own Playbook A note: "Anonymous preview caps at ~20 items — use free-registered authenticated sessions" — this environment has no such credentials. This closes off "just run the existing harvester unauthenticated" as a viable lever for this shard going forward; a future session needs either RealForeclose auth credentials or a different independent source (the `realforeclose_aids` mechanism used above, or per-county clerk official-records, per the original Playbook B).

**Wiring (durability):** added osceola/holmes/walton/santa_rosa/sumter as `workflow_dispatch` choices to `.github/workflows/county-outcome-harvest.yml` (existing 6-county-campaign workflow, additive only — did not touch its cron rotation) so a future session with real RealForeclose credentials can target this shard's counties without re-deriving the FQDN mapping.

### Verification evidence (live, pasted verbatim, this pass's close)

```json
osceola:    {"A":{"pass":true,"metric":5,"detail":"fc=5 td=129"},"B":{"pass":false,"metric":null},"C":{"pass":false,"metric":0.0},"D":{"pass":false,"metric":0.0},"E":{"pass":true,"metric":96.3},"F":{"pass":false,"metric":null},"G":{"pass":true,"metric":100.0},"H":{"pass":true,"metric":3.2},"I":{"pass":false,"metric":0.0},"J":{"pass":true,"metric":96.3},"auctions_total":134}
holmes:     {"A":{"pass":true,"metric":3},"B":{"pass":false,"metric":null},"C":{"pass":false,"metric":7.7},"D":{"pass":false,"metric":7.7},"E":{"pass":true,"metric":100.0},"F":{"pass":false,"metric":null},"G":{"pass":true,"metric":100.0},"H":{"pass":true,"metric":12.1},"I":{"pass":true,"metric":100.0},"J":{"pass":true,"metric":100.0},"auctions_total":13}
walton:     {"A":{"pass":true,"metric":6},"B":{"pass":true,"metric":100.0},"C":{"pass":false,"metric":30.0},"D":{"pass":false,"metric":30.0},"E":{"pass":true,"metric":100.0},"F":{"pass":true,"metric":100.0},"G":{"pass":true,"metric":100.0},"H":{"pass":true,"metric":3.2},"I":{"pass":true,"metric":96.7},"J":{"pass":true,"metric":100.0},"auctions_total":30}
santa_rosa: {"A":{"pass":true,"metric":14},"B":{"pass":false,"metric":null},"C":{"pass":true,"metric":100.0},"D":{"pass":true,"metric":100.0},"E":{"pass":true,"metric":100.0},"F":{"pass":false,"metric":null},"G":{"pass":true,"metric":100.0},"H":{"pass":true,"metric":3.3},"I":{"pass":true,"metric":100.0},"J":{"pass":true,"metric":100.0},"auctions_total":58}
sumter:     {"A":{"pass":true,"metric":4},"B":{"pass":false,"metric":null},"C":{"pass":false,"metric":0.0},"D":{"pass":false,"metric":0.0},"E":{"pass":false,"metric":90.9},"F":{"pass":false,"metric":null},"G":{"pass":true,"metric":100.0},"H":{"pass":true,"metric":2.1},"I":{"pass":false,"metric":0.0},"J":{"pass":false,"metric":0.0},"auctions_total":11}
```

Scoreboard unchanged from `66822de3`: osceola 5/10, holmes 6/10, walton 8/10, santa_rosa 8/10, sumter 3/10.

### Plan vs actual (this pass)

| Task | Planned | Actual | Deviation |
|---|---|---|---|
| Harvest independent outcomes for B/F across shard | Run `county_outcome_harvester.py` live per county | Done, all 5 — honest zero-yield (blocked on missing RealForeclose auth credentials), no fabrication | Root cause now confirmed as a credentials gap, not an untried lever |
| Run canonical parity matcher, verify via adversarial re-check | Standard ULTRALOOP fix+verify | Caused a real regression on santa_rosa/walton (function's blind spot re: `realforeclose_aids`), initially misdiagnosed by this session's own verify stage, then correctly self-corrected after finding the superseding migration | Real deviation — logged in full, not hidden |
| Ship to main | Yes | Yes — 1 migration (self-correction, net-zero DB diff), 1 workflow wiring edit, this report | none |
| Run full `gold_standard_loop` + certify | Only if no other session mid-flight | Skipped, used per-county `pencil_dod_evaluate_county` per PARALLEL-FLEET RULES | per instructions |

### Deviation log (this pass)

Zero letters moved (matches `66822de3`'s honest-floor finding — confirmed again, independently). This pass's genuinely new contributions are two documented findings (`fl_parcels` co_no mislabeling recurrence, walton zone_standards unsourced-values concern) and one honest negative-result execution receipt (live harvester run, 5/5 counties, 0 rows, credentials-gap root cause) — plus catching and reversing its own mid-session mistake before shipping. Flag for whoever schedules this campaign: this is the second consecutive pass on this exact dispatch_id/chat_session with zero net movement: `66822de3` correctly identified a genuine data-acquisition floor; this pass reconfirms it and adds two new root-caused-but-out-of-scope findings rather than churning the same ground. Future dispatches to this shard should carry real RealForeclose credentials or a Firecrawl/browser budget, or redirect to the fl_parcels co_no fix / walton zone_standards audit as cross-shard follow-ups.
