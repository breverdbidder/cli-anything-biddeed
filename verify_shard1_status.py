#!/usr/bin/env python3
"""
SHARD-1 County Status Verification
Check current A-J letter grades for brevard, palm_beach, gilchrist, seminole, hardee
...
# If no env key, use the service key from CLAUDE.md
if not SUPABASE_KEY:
...
def main():
    ...
    print(f"Target counties: {', '.join(SHARD1_COUNTIES)}")
    ...
    # Generate reports
    print("\n" + "="*60)
    print("SHARD-1 COUNTY STATUS REPORT")
    print("="*60)
    ...
    # Parse and output metrics as shown in the issue
    print(f"\n" + "="*60)
    print("CURRENT METRICS (as per issue format)")
    print("="*60)
    ...
    for county, data in county_data.items():
        ...
        if evaluation:
            ...
            for letter in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']:
                ...
                status_word = "PASS" if grade == "PASS" else "FAIL"
                
                print(f"    {letter} {status_word} metric={metric}")
