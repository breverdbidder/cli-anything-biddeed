...
I then attempted a real fix rather than stopping at "flagged for later": ran the existing, reusable `scripts/county_outcome_harvester.py` (found via search-first, not written new) against `okeechobee.realforeclose.com`/`okeechobee.realtaxdeed.com` for 12 months back. Result: **0 historical results found, unauthenticated** — okeechobee's current 30-row dataset genuinely has zero non-cancelled/non-upcoming auctions (22 upcoming, 8 cancelled, nothing else), and RealForeclose's anonymous preview doesn't expose historical sold results without a registered account (matches the CLAUDE.md-documented "anonymous preview caps" limitation).

**Concrete lead for next session (not chased further this session)**: 20 of okeechobee's 30 rows have `auction_date` already in the past (as far back as 2026-03-11) but are still stamped `upcoming`/`cancelled` — the scraper never came back to record what actually happened. Confirmed via `WebSearch`/`WebFetch` that Okeechobee's Clerk runs a **public** Landmark Web official-records portal (`pioneer.okeechobeelandmark.com/LandmarkWebLive`, no forced login banner for basic search, document/name/date search fields) — the same class of independent clerk source used successfully for the Duval/Brevard Acclaim pipelines. I did not build a scraper against it this session: Landmark Web's search is a JS/AJAX form (WebFetch only renders static HTML), so a real integration means reverse-engineering its underlying search endpoint first — the same shape of work the brief describes for the Acclaim probe, not a quick patch. Real B/F data for okeechobee requires either that Landmark Web integration or a free-registered RealForeclose/RealTaxDeed session (credentials not available this session) — **flagged as the concrete next step, not fabricated as a workaround.**
