...
| `mcp_usage_log` table | Did not exist earlier in this session (confirmed 404 on REST HEAD, per `INCIDENT_RESPONSE_PLAN.md` §3 correction note). A concurrent session shipped a table by this exact name later the same day (`20260803_mcp_usage_anomaly_detection.sql`) — but for behavioural-anomaly detection (volume spikes, off-hours calls, new-county access), not the billing/usage-metering purpose the original brief meant. The billing metering tables remain `taxi_meter_streams` / `taxi_meter_tools`. | LOG answers cite both tables by their actual, distinct purposes rather than treating them as interchangeable. |
...
| LOG-02 Usage / access logging | YES | `taxi_meter_streams` / `taxi_meter_tools` — confirmed live — record per-call MCP tool usage and billing metering. A separate `mcp_usage_log` table (shipped by a concurrent session the same day this document was written) feeds behavioural anomaly detection specifically — see LOG-03. |
| LOG-06 Behavioural anomaly detection | YES | Pure-SQL `pg_cron` pipeline against `mcp_usage_log` with a 30-day rolling baseline, detecting volume spikes, off-hours high-tier tool calls, first-time county access, and bulk county sweeps — feeds the same `security_events`/Telegram alert pipeline as other detection sources. Shipped same-day as this document (commit `f97d2183`); not independently re-tested in this session beyond confirming the commit and migration exist. |
