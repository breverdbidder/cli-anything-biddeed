...
**Status:** RATIFIED 2026-04-12 · MANDATORY — no dossier is complete until every checkpoint is ticked with verified evidence.
...
- **v1** (Apr 10 2026): Initial 173 checkpoints across 13 phases, committed to this path.
- **v1.2** (Apr 12 2026): +22 ratified delta checkpoints marked ✦ *v1.2*. Ratified by Ariel via everest-battle-cards/research/ci-protocol-v1-2-195cp/ card Section 6. Total: 195 checkpoints across 13 phases.

**Delta summary (v1 → v1.2):**
- Phase 0: +2 (0.9 tmux lock, 0.10 BrandGuard scanner)
- Phase 1: +2 (1.16 Twitter/X, 1.17 GitHub org)
- Phase 2: +3 (2.17 Lighthouse, 2.18 axe, 2.19 perf trace)
- Phase 3: +2 (3.14 OpenAPI, 3.15 webhook)
- Phase 4: +2 (4.9 ACV, 4.10 payment processor)
- Phase 5: +2 (5.14 PCT, 5.15 bar complaints)
- Phase 6: +2 (6.12 podcast, 6.13 conference)
- Phase 7: +2 (7.12 framework version, 7.13 package lock)
- Phase 8: +1 (8.60 LLM visibility / GEO)
- Phase 9: +2 (9.6 velocity, 9.7 roadmap scraping)
- Phase 10: +1 (10.6 post-fix re-run evidence)
- Phase 11: +1 (11.6 LinkedIn post draft)
- Phase 12: +0
- **Total +22 → 195 checkpoints**
...
### v1.2 Delta Checkpoints (Ratified Apr 12 2026)

- [ ] **0.9 Hetzner tmux session lock claimed** ✦ *v1.2*
  - Artifact: tmux session ci-dossier-&lt;slug&gt; created
  - Evidence: `tmux ls shows exactly one ci-dossier session`
  - Verification: `tmux has-session -t ci-dossier-&lt;slug&gt;`

- [ ] **0.10 BrandGuard scanner installed + calibrated** ✦ *v1.2*
  - Artifact: BrandGuard CLI returns version
  - Evidence: `scan of known-good zonewise.ai returns 0 violations`
  - Verification: `brandguard --version && brandguard scan https://zonewise.ai`
...
- [ ] **1.16 Twitter/X founder + company signals captured** ✦ *v1.2*
  - Artifact: X profile data + engagement trajectory
  - Evidence: `ci_dossiers.twitter_signals populated`
  - Verification: `firecrawl_scrape x.com/&lt;handle&gt;`

- [ ] **1.17 GitHub org captured (if exists)** ✦ *v1.2*
  - Artifact: org profile + top repos + contributor count
  - Evidence: `ci_dossiers.github_org populated or NULL with reason`
  - Verification: `curl https://api.github.com/orgs/&lt;slug&gt;`
...
- [ ] **2.17 Lighthouse score per URL** ✦ *v1.2*
  - Artifact: JSON Lighthouse report per URL
  - Evidence: `ci-evidence/&lt;slug&gt;/lighthouse/&lt;url&gt;.json exists, scores captured`
  - Verification: `npx lighthouse &lt;url&gt; --output=json`

- [ ] **2.18 axe-core WCAG 2.1 AA violations per URL** ✦ *v1.2*
  - Artifact: axe JSON per URL
  - Evidence: `ci-evidence/&lt;slug&gt;/axe/&lt;url&gt;.json exists`
  - Verification: `@axe-core/playwright injectAxe + analyze`

- [ ] **2.19 Performance trace per URL** ✦ *v1.2*
  - Artifact: Chrome DevTools trace per URL
  - Evidence: `ci-evidence/&lt;slug&gt;/trace/&lt;url&gt;.json exists`
  - Verification: `page.context.tracing.start/stop`
...
- [ ] **3.14 OpenAPI / Swagger spec auto-detection** ✦ *v1.2*
  - Artifact: parsed spec if discovered
  - Evidence: `ci-evidence/&lt;slug&gt;/openapi.json if found`
  - Verification: `probe /openapi.json /swagger.json /api-docs`

- [ ] **3.15 Webhook endpoint detection** ✦ *v1.2*
  - Artifact: webhook URLs discovered
  - Evidence: `ci_dossier_endpoints rows with kind='webhook'`
  - Verification: `parse docs + JS bundles for webhook patterns`
...
- [ ] **4.9 Annual contract value (ACV) inferred** ✦ *v1.2*
  - Artifact: ACV range estimate with confidence
  - Evidence: `ci_dossiers.acv_range jsonb`
  - Verification: `triangulate from funding + headcount + pricing`

- [ ] **4.10 Payment processor detected** ✦ *v1.2*
  - Artifact: Stripe / Chargebee / Recurly / other
  - Evidence: `ci_dossiers.payment_processor populated`
  - Verification: `parse checkout page + network HAR`
...
- [ ] **5.14 PCT international application search** ✦ *v1.2*
  - Artifact: PCT applications (separate from EPO/WIPO)
  - Evidence: `ci_dossier_patents rows with source='pct'`
  - Verification: `patentscope PCT filter`

- [ ] **5.15 Bar association complaints / regulatory complaints** ✦ *v1.2*
  - Artifact: any formal complaints against team
  - Evidence: `ci_dossier_regulatory rows with type='complaint'`
  - Verification: `state bar + CFPB + FTC complaint DBs`
...
- [ ] **6.12 Podcast appearances (founders + execs)** ✦ *v1.2*
  - Artifact: podcast episodes with dates
  - Evidence: `ci_dossier_podcast_appearances rows`
  - Verification: `listen notes + apple podcasts API`

- [ ] **6.13 Conference speaking + event presence** ✦ *v1.2*
  - Artifact: conference talks, booth presence
  - Evidence: `ci_dossier_events rows`
  - Verification: `conference site scraping + LinkedIn posts`
...
- [ ] **7.12 Runtime framework version detection** ✦ *v1.2*
  - Artifact: exact version strings from bundles
  - Evidence: `ci_dossiers.framework_versions jsonb`
  - Verification: `bundle regex + meta tag inspection`

- [ ] **7.13 Package manager lock inspection (if sourcemaps leak)** ✦ *v1.2*
  - Artifact: npm/yarn/pnpm lock hints from sourcemap paths
  - Evidence: `ci_dossiers.package_manager_hints populated`
  - Verification: `sourcemap directory structure analysis`

---

## PHASE 8 — Volume IX: Marketing, SEO, GEO, Customer Behavior, Lead Gen, Recurring Revenue

**This is the newly added phase per Ariel's April 10 2026 correction.**
...
- [ ] **8.60 LLM visibility (GEO — generative engine optimization)** ✦ *v1.2*
  - Artifact: ChatGPT + Claude + Perplexity + Gemini brand mention frequency
  - Evidence: `ci_dossiers.llm_visibility jsonb`
  - Verification: `run 10 seed prompts in each LLM, count brand mentions`
...
- [ ] **9.6 Feature velocity (commit cadence proxy)** ✦ *v1.2*
  - Artifact: feature releases per month trend
  - Evidence: `ci_dossiers.feature_velocity jsonb`
  - Verification: `changelog / release notes parse`

- [ ] **9.7 Roadmap signal scraping** ✦ *v1.2*
  - Artifact: public roadmap if exists
  - Evidence: `ci_dossiers.public_roadmap populated`
  - Verification: `parse /roadmap /changelog /what's-new`
...
- [ ] **10.6 Post-fix re-run evidence preserved** ✦ *v1.2*
  - Artifact: diff between failed and passing run saved
  - Evidence: `ci-evidence/&lt;slug&gt;/eg14/iteration-&lt;n&gt;/ exists per iteration`
  - Verification: `bucket listing check`
...
- [ ] **11.6 LinkedIn bake-off post draft generated** ✦ *v1.2*
  - Artifact: one-post markdown draft
  - Evidence: `deliverables/&lt;slug&gt;/linkedin-post.md exists`
  - Verification: `template render`
...
## Time & Credit Budget Per Competitor

| Phase | Est Duration | Est Credits | Notes |
|---|---|---|---|
| 0 (one-time) | 15 min | 5 | Infrastructure |
| 1 | 15 min | 0 | Free data sources |
| 2 | 35 min | 200 | Playwright visual (credit-heavy) |
| 3 | 20 min | 30 | API discovery |
| 4 | 10 min | 20 | Pricing/business model |
| 5 | 20 min | 40 | Legal/IP deep |
| 6 | 20 min | 50 | Customer/market |
| 7 | 10 min | 10 | Tech stack |
| 8 | 45 min | 120 | SEO + GEO + behavior + lead gen + recurring rev |
| 9 | 5 min | 0 | Feature mapping |
| 10 | 5 min | 0 | Gate execution |
| 11 | 10 min | 0 | Deliverables from SQL |
| 12 | 5 min | 0 | Session close |
| **Total per competitor** | **~3.5 hrs** | **~475** | |
| **Total for 11** | **~38.5 hrs** | **~5,225** | Well within 100,284 budget |
...
**Version:** 1.0
**Date:** April 10, 2026
**Authority:** Approved by Ariel Shapira as mandatory per this message

---

## Footer · v1.2 Totals

| Phase | v1 | v1.2 Delta | v1.2 Total |
|-------|----|-----------|-----------|
| 0  · Infrastructure Readiness     |  8 | +2 | 10 |
| 1  · Surface Intelligence         | 15 | +2 | 17 |
| 2  · Visual & Interactive Capture | 16 | +3 | 19 |
| 3  · API & Endpoint Discovery     | 13 | +2 | 15 |
| 4  · Service Scope & Pricing      |  8 | +2 | 10 |
| 5  · Legal, IP, Moat              | 13 | +2 | 15 |
| 6  · Customer & Market Intel      | 11 | +2 | 13 |
| 7  · Tech Stack Fingerprinting    | 11 | +2 | 13 |
| 8  · Marketing, SEO, GEO, Revenue | 59 | +1 | 60 |
| 9  · Feature Extraction + Patent  |  5 | +2 |  7 |
| 10 · EG14 Gate Execution          |  5 | +1 |  6 |
| 11 · Deliverables from Supabase   |  5 | +1 |  6 |
| 12 · Memory Update + Session Close|  4 | +0 |  4 |
| **TOTAL** | **173** | **+22** | **195** |

---

*CI Dossier Checkpoint Protocol v1.2 ratified 2026-04-12. This is the SSOT. Every battle card at `breverdbidder.github.io/everest-battle-cards/` must pass all 195 checkpoints before EG14 gate execution. No deliverable until all 13 phases green and EG14 passes 14/14 on production domain.*
