...
B4. **THE STEP THAT BREAKS EVERYTHING IF SKIPPED** — click **PUBLISH APP** so publishing status reads **In production**, not Testing. Testing = silent death in 7 days with invalid_grant. Screenshot the "In production" state.
B5. Credentials -> Create credentials -> OAuth client ID -> **Desktop app** -> name `biddeed-publisher-desktop` -> Download JSON. Keep local, never commit.
...
C3. In Cloud Console -> Credentials -> biddeed-publisher-desktop -> Authorised redirect URIs -> add exactly `https://developers.google.com/oauthplayground` -> Save. Without this, C4 fails with redirect_uri_mismatch.
