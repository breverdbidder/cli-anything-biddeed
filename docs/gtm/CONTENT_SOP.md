...
| P6 | Patent phrasing | Default: no mention at all (background only). If unavoidable, exactly one of: `provisional patent application, 14 claims` or `provisional patent application` (number-free). Any other `/patent/i` hit FAILS, including "12", "14 patents", "patented", "patent-pending". Live `/` currently FAILS this (§1) | regex | GAP → SPR-01 (check), SPR-03 (fix) |
| P7 | Canon strings | Tagline, product line, signature, hero stack, banner, YouTube description only in their canon form (case and punctuation exact: `For Everyone. Everywhere.`). Retired line banned (P2) | regex against a `CANON_STRINGS` list read from the canon file | GAP → SPR-01 |
| P8 | Energy rules | ≤ 1 `!` in visible text per page; no "act now" / "limited time" / "hurry" / "don't miss"; no competitor brand; passive "is provided" / "leveraged" → WARN | regex on visible text (strip scripts/styles first — raw HTML counts `!important`) | GAP → SPR-01 |
| P9 | Positioning | "US county" / "nationwide" per canon rule 5; never "all 50 states" or "3,000 counties" until live (tagline compliance note); always say these are real auctions run by US counties | regex + judge | GAP → SPR-01 |
...
| P17 | Status-quo respect | No line shames the reader's current process. Regex seed: "you're doing it wrong", "still using spreadsheets", "if you're still", "amateur", "rookie mistake", "most bidders are too lazy/dumb" → FAIL. Enemy = guesswork/friction, never the bidder (N4) | regex on visible text + judge | GAP → SPR-01 (`CONTEMPT_RE`) |
| P18 | Vividness | Buzzword denylist → FAIL: "comprehensive solution", "next-gen", "cutting-edge", "streamlined", "seamless", "revolutionary", "industry-leading", "best-in-class", "leverage", "synergy", "robust platform". Founder-voice assets must contain ≥ 1 concrete auction scene or live number (N2/N5) | regex denylist + judge | GAP → SPR-01 (`BUZZWORD_RE`) |
