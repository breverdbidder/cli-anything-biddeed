...
| P19 | Parity gates before publish | Every PR that touches a page runs `scripts/ui-audit/audit.py` against a faithful preview (biddeed-web: the branch build; Worker: live HTML with the branch CSS applied) at 1440 + 390 and pastes the eight-gate table in the PR body; any red gate (render, palette, contrast, copy, data, function, SEO, a11y) = not mergeable. After deploy, `ui-audit.yml` re-runs on the live URL and posts to #19829. Standard: docs/design/PARITY_PRD.md sections 1, 7, 8 | harness + judge | EXISTS (2026-09-04) |
