...
-- ROOT CAUSE of the recurring C/D ghost-duplicate reseed (VERIFIED live 2026-07-30):
-- biddeed.flow_card_to_mca() derives sale_type purely from pipeline.tier1_card_raw.
-- source_platform ('realtaxdeed' -> tax_deed, else -> foreclosure). Okeechobee has no
-- separate taxdeed_platform/taxdeed_url configured (pipeline.counties confirmed both
-- NULL) -- its single combined okeechobee.realforeclose.com calendar serves BOTH
-- foreclosure and tax-deed items under one source_platform='realforeclose' tag. Every
-- TD-docket item therefore gets force-classified sale_type='foreclosure', and since
-- promote_upcoming_tier1_cards() (cron 204, gold-calendar-parity-cycle) reprocesses
-- ALL unexpired tier1_card_raw rows every cycle with no processed-flag, any time the
-- resulting mislabeled row is deleted it gets silently re-inserted within minutes.
--
-- FIX: case_number is a statewide Florida clerk docket-type signal independent of
-- which URL/scraper found it -- a "<year>TD<digits>" case number is a Tax Deed
-- docket and is NEVER used for a real foreclosure (CA) case. Verified live before
-- applying: this exact pattern (^[0-9]{4}TD[0-9]+$) appears in multi_county_auctions
-- for okeechobee (32 rows, all mislabeled foreclosure) and manatee (3 rows, already
-- correctly tax_deed) and NOWHERE ELSE as sale_type='foreclosure' across the entire
-- table -- so this override cannot misclassify any existing real foreclosure case in
-- any county. Additive only: source_platform='realtaxdeed' still classifies as
-- tax_deed for every other case-number shape; only the explicit TD-docket pattern is
-- promoted ahead of the platform-based default.
