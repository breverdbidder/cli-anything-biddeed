-- SHARD-2: re-merge of the brevard-I tax_account fallback fix.
--
-- Between the first application of this fix (20260702_shard2_brevard_i_tax_account_fallback.sql)
-- and this migration, another concurrent shard replaced pencil_dod_evaluate_county with a
-- (correct, welcome) fix to criterion F -- tier1_sold now requires sold_amount IS NOT NULL too,
-- fixing the >100% anomaly (brevard F was 934.6%, now 98.9%). Because both shards ship via
-- full CREATE OR REPLACE FUNCTION against a shared function with no locking, that push silently
-- reverted the I-criterion tax_account fallback (brevard I dropped back to 35.2%).
--
-- This migration re-applies the I fix ON TOP of the current live function (which already
-- contains the F fix), so both survive together. No other lines are touched.
--
-- VERIFIED before applying: current live function's zc CTE selects only parcel_id (no
-- tax_account), confirmed via pg_get_functiondef immediately before writing this file.
