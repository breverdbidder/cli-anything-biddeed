-- GOLD STANDARD shard-3 lake (dispatch 77ac9cef): fix C/D live regression
-- caused by the same-day clerk_ssot rollout (scripts/clerk_ssot/run_parity.py,
-- migration 20260810_clerk_ssot_task4_render_gate.sql).
--
-- ROOT CAUSE (confirmed live via Management API query, 2026-08-10T16:xx UTC):
-- clerk_ssot marks clean-matched rows parity_status='PARITY_OK' (and newly
-- reconciled rows 'CLERK_VERIFIED'), NOT 'matched_clean'/'matched_divergent'
-- with a 'tier1%' parity_source prefix. pencil_dod_evaluate_county's C/D
-- FILTER clauses only recognized the old tier1-prefixed vocabulary, so once
-- clerk_ssot ran against lake (and 8 other counties: brevard, gadsden,
-- highlands, okeechobee, st_johns, suwannee, union, wakulla -- see PARSERS
-- dict in run_parity.py), the previously-passing C/D metrics collapsed:
-- lake C 94.1->31.7 (matched_clean 111->38), D 99.2->36.7 (matched_any 117->44).
-- Renaming the DATA back to the old vocabulary is not an option: the render
-- gate public.v_property_card_verified (Task 4.2, same-day migration) filters
-- literally on parity_status IN ('CLERK_VERIFIED','PARITY_OK') -- renaming
-- those values would break the live customer-facing property-card view.
-- Fix is additive-only at the evaluator: recognize PARITY_OK/CLERK_VERIFIED
-- as matched_clean (they are clerk-source-of-truth confirmed clean matches
-- per the run_parity.py docstring: "mark clean matches PARITY_OK"), and
-- additionally recognize CLERK_SSOT_CANCELLED as matched_any (not clean --
-- it represents a divergence that clerk_ssot found and corrected, which is
-- the same class as matched_divergent, not a no-divergence-ever clean match).
-- PHANTOM_NOT_ON_CLERK and unrelated legacy parity_status values (e.g.
-- 'mca_only' from the owner-name E-linkage pipeline) are intentionally left
-- unmatched -- they are genuine anomalies, not reconciled agreement with the
-- clerk source of truth, and counting them would be ghost-success.
--
-- Zero data mutation. Purely additive WHERE-clause recognition, so this can
-- only raise C/D for the 9 clerk_ssot counties and has no effect on any
-- other county's score.
...
WITH a AS (
  SELECT count(*) AS auctions_total,
         ...
         count(*) FILTER (WHERE (parity_status IN ('matched_clean','matched_divergent') AND parity_source LIKE 'tier1%%')
                             OR parity_status IN ('PARITY_OK','CLERK_VERIFIED','CLERK_SSOT_CANCELLED')) AS matched_any,
