-- SHARD-2 (brevard/franklin/lee): fix criterion I card_complete parcel-match.
--
-- pencil_dod_evaluate_county()'s `zc` CTE matched multi_county_auctions.parcel_id
-- only against v_zoning_gold_standard_card.parcel_id (formatted STRAP key). Brevard
-- stores a bare BCPAO tax-account number in multi_county_auctions.parcel_id, which
-- lives in v_zoning_gold_standard_card.tax_account instead -- the join missed 4461
-- otherwise field-complete rows (address+geo+value all present).
--
-- Fix: OR-fallback matching tax_account, same pattern gold_standard_loop() already
-- uses (vc.parcel_id = a.parcel_id OR vc.tax_account = a.parcel_id). Additive-only
-- (widens the match set), so no other county's I metric can regress.
--
-- VERIFIED via live simulation before applying: brevard card_complete 2527->6932
-- of 7187 (35.2%->96.5%), crossing the 95% threshold.
