-- Fix: gold_standard_loop H-criterion uses GREATEST for last_seen
-- Problem: COALESCE(last_changed_at, last_seen_at, ...) picks stale last_changed_at first
-- because trigger tg_freshness_capture resets last_changed_at to OLD value when content
-- hash is unchanged. Manual freshness refresh via last_seen_at is ignored.
-- Fix: use GREATEST to pick the most recent timestamp across all candidates.
-- Architecture: v9_3_greatest_freshness_fix
