-- Extend sync_credential_from_gha's whitelist so the existing
-- Option-C GHA->vault sync (sync-credentials-to-vault.yml) can propagate
-- the confirmed-working Telegram bot credential into vault.secrets.
-- BIDDEED_BOT_TOKEN / BIDDEED_BOT_CHAT_ID are the verified-working pair
-- (used by .github/workflows/telegram-notify.yml, sent live messages
-- 2026-08-03). TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID also exist as GH
-- secrets but are unverified and intentionally left out of this map.
