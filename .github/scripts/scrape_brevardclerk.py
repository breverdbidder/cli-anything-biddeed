#!/usr/bin/env python3
"""Brevard Tier1 v9.9 - PROPER pagination via img[alt='Next Page'] click action.
Iterates all 12 pages of 'Auctions Closed or Canceled' on the RealAuction PREVIEW view."""
...
# Both pagination images on the page have alt="Next Page". The second one (in
# Auctions Closed section) is the one we want. We click the LAST matching element
# so we advance the closed-auctions list.
NEXT_SELECTOR = 'img[alt="Next Page"]:last-of-type'
# Fallback selector if :last-of-type isn't supported
NEXT_SELECTOR_FALLBACK = 'img[alt="Next Page"]'
...
def extract_cards(md, schema_by_field, status_map):
    ...
    cards = []
    # Only consider cards AFTER "Auctions Closed or Canceled" header to avoid Currently-Selling section
...
run_id = rpc('scrape_log_start', {
    ...
    'p_triggered_by':'gha_workflow_dispatch_v9_9_paginated',
})
print(f'>>> v9.9 PAGINATED, run={run_id}')
...
try:
    ...
    for page_idx in range(MAX_PAGES):
        print(f'\n--- PAGE {page_idx+1} ({page_idx} clicks) ---')
        md = firecrawl_with_clicks(PREVIEW_URL, page_idx)
        if not md:
            print(f'  empty markdown, stop')
            break
        ...
        for c in new:
            ...
            all_cards.append(c)
        page_stats.append({'page':page_idx+1,'md_chars':len(md),'cards':len(cards),'new':len(new)})
        print(f'  md={len(md):,} cards={len(cards)} new={len(new)} total={len(all_cards)}')
        if len(new) == 0 and page_idx > 0:
            print(f'  → no new parcels, stopping pagination')
            break
    ...
    summary = {
        'parser':'v9.9_paginated_alt_next',
