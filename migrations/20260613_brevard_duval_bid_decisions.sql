-- BREVARD DUVAL Gold Standard Letter J: Bid Decisions Migration
...
DROP POLICY IF EXISTS "Enable all for service role" ON bid_decisions;
CREATE POLICY "Enable all for service role" ON bid_decisions
  FOR ALL USING (auth.role() = 'service_role');

-- Public read access for authenticated users
DROP POLICY IF EXISTS "Enable read for authenticated users" ON bid_decisions;
CREATE POLICY "Enable read for authenticated users" ON bid_decisions
  FOR SELECT USING (auth.role() = 'authenticated');

-- Specific policy for brevard and duval counties
DROP POLICY IF EXISTS "Enable brevard duval counties" ON bid_decisions;
CREATE POLICY "Enable brevard duval counties" ON bid_decisions
  FOR ALL USING (county_slug IN ('brevard', 'duval'));
...
DROP TRIGGER IF EXISTS bid_decisions_validate_factors ON bid_decisions;
...
CREATE OR REPLACE VIEW v_bid_decisions_j_metrics_brevard_duval AS
SELECT 
...
COMMENT ON VIEW v_bid_decisions_j_metrics_brevard_duval IS 'J letter metrics for brevard and duval: bid_decisions completeness per Shapira Formula contract';

-- Brevard Duval specific verification query for ULTRALOOP protocol
CREATE OR REPLACE FUNCTION brevard_duval_j_verification(county_name TEXT)
RETURNS TABLE(
  county TEXT,
...
BEGIN
  ...
  FROM v_bid_decisions_j_metrics_brevard_duval v
  WHERE v.county_slug = county_name;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION brevard_duval_j_verification IS 'ULTRALOOP verification function for brevard/duval J letter status with evidence';

-- Update county slugs to match the table conventions
INSERT INTO fl_counties (co_no, name, fips_code, slug, region) VALUES 
  (9, 'Brevard', '12009', 'brevard', 'central'),
  (31, 'Duval', '12031', 'duval', 'northeast')
...
VALUES (
  '20260613_brevard_duval_bid_decisions',
  NOW(),
  'AUTOPILOT RUN 21: bid_decisions table setup for J letter (brevard, duval) - Shapira Formula pipeline'
