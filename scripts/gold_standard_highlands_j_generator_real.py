#!/usr/bin/env python3
"""
Gold Standard highlands letter J (Shapira deal thesis completeness) — REAL
generator, forked verbatim (methodology) from
scripts/gold_standard_shard9_osceola_run255f_j_generator_real.py, the
...
for rows with no real value on file. This script additionally self-stages
the model artifacts from Supabase Storage (pattern from
scripts/suwannee_shard4_c40bb245_j_generator_extend.py) since /tmp/shapira
is not guaranteed to be pre-populated.

highlands IS in the v14 training corpus's county_target_encoding_map (real
trained rate 0.832, confirmed live from
...
join, re-derived directly against multi_county_auctions vs bid_decisions):
exactly 18 highlands case_numbers fail the deal_complete definition, all 18
with zero existing bid_decisions row (first-time fill, not a stale/bucketed
row to purge). Of those 18:
  - 17 have a real parcel_id AND real assessed_value/market_value already
    on file in multi_county_auctions (property-appraiser sourced) --
    processed here via the existing real_arv() assessed/market fallback
    (no parcel_valuations comps_cma_bulk rows exist for any of the 17
    parcel_ids, confirmed live -- 0 rows in `parcels` table for these
    parcel_ids).
  - 1 (25000484GCAXMX) has NULL parcel_id, NULL property_address, NULL
    assessed_value, NULL market_value -- no real data on file anywhere.
    Correctly skipped by the existing skipped_no_real_value branch
...
# (highlands IS in the 45-county training corpus, no fallback needed).
COUNTY_TARGET_ENC = 0.832
PIPELINE_VERSION = "highlands_j_shapira_v14_real"
