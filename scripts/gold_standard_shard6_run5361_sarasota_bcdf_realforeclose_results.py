#!/usr/bin/env python3
"""GOLD STANDARD shard-6 run5361 -- sarasota B/C/D/F fix (RealForeclose
Auction Results Report), foreclosure lane residual gap.

Root cause (confirmed live 2026-07-20 via pencil_dod_evaluate_county('sarasota')):
  Of sarasota's 93 DoD-scoped foreclosure rows, a prior session
  (shard10_run3497) already independently matched 34 via a live RealAuction
  AJAX calendar harvest (parity_source='tier1_realauction_live:sarasota:
  shard10_run3497' -- confirmed real, varied tier1_sold_amount values, not
  fabricated) and inserted 22 foreclosure_outcomes rows. This script targets
  the RESIDUAL 61-row gap (25 completed-but-unverified + 17 cancelled + 19
  upcoming-but-past-dated, all confirmed past their auction_date) using the
  SAME proven technique already used for hendry/santa_rosa/sarasota-tax_deed
  (scripts/shard2_hendry_bf_realtaxdeed_results.py,
  scripts/shard7_run3679_santa_rosa_bf_realforeclose_results.py,
  scripts/gold_standard_shard6_run5361_sarasota_bcdf_realtaxdeed_results.py):
  the authenticated sarasota.realforeclose.com "Auction Results Report"
  (report_id=18 -- CONFIRMED live for sarasota by direct probe, same
  report_id as hendry/santa_rosa/sarasota-taxdeed).
  ...
  INDEPENDENT source from our own pre-sale calendar-sweep scraper. Confirmed
  live 2026-07-20: sarasota's foreclosure grid returns case_number as an
  HTML anchor in cell[1] (<a href="https://secure.sarasotaclerk.com/
  viewTiff.aspx?...">2025 CA 001761 NC</a>), 15 cells total (NO
  cert_or_deed_no column, unlike the tax_deed grid's 16 cells -- confirmed
  live, not assumed).
...
'tier1_realforeclose_results_report:sarasota:gold_standard_shard6_run5361' and
...
Usage:
  ...
  python3 scripts/gold_standard_shard6_run5361_sarasota_bcdf_realforeclose_results.py --dry-run
"""
...
DATA_SOURCE_TAG = "tier1_realforeclose_results_report:sarasota:gold_standard_shard6_run5361"
PARITY_SOURCE_TAG = "tier1_realforeclose_live:sarasota:gold_standard_shard6_run5361"
...
# sarasota's foreclosure grid returns case_number as an HTML anchor in
# cell[1] (confirmed live 2026-07-20), 15 cells total -- SAME layout as
# santa_rosa's RealForeclose instance, NO cert_or_deed_no column (that's a
# tax_deed-only field, confirmed live).
...
def main():
    log("=== GOLD STANDARD SHARD-6 RUN5361 SARASOTA B/C/D/F FIX (RealForeclose Auction Results Report) ===")
    ...
    # matching pencil_dod_evaluate_county's own WHERE clause) -- sarasota has
    # 1204 TOTAL foreclosure rows but only 93 are in DoD scope; the other
    # 1111 are propertyonion litmus rows that must never be touched here.
    # NOTE: PostgREST's `not.eq` is SQL `<>`, which is NULL (excludes the row)
    # for NULL data_source -- confirmed live 2026-07-20 this silently dropped
    # 35 in-scope NULL-data_source rows (58 fetched instead of 93). Fixed by
    # explicitly OR-ing in the `data_source.is.null` leg.
    mca_rows = rest_get(
    ...
    log(f"Fetched {len(mca_rows)} sarasota DoD-scoped foreclosure multi_county_auctions rows", "VERIFIED")
    ...
    log(f"Matched {len(matched)} sarasota foreclosure rows to RealTaxDeed results with "
    ...
    for row, rr in matched:
        if DRY_RUN:
            ...
            # sarasota.realforeclose.com Auction Results Report (report_id=18),
    ...
    print("SELECT county, sold_amount_source, COUNT(*) FROM multi_county_auctions "
          "WHERE county='sarasota' AND sale_type='foreclosure' AND sold_amount IS NOT NULL "
