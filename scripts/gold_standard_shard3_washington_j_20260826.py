#!/usr/bin/env python3
"""
Gold Standard shard-3 -- REAL J generator for washington's 2026-08-25 tax-deed
batch remainder, forked verbatim (methodology) from
scripts/architect_triage_19313_washington_j_generator_20260818.py (the 2026-08-18
session that did this exact thing for washington's prior batch), which itself
forks scripts/gold_standard_shard1_a3eafa08_washington_j_generator_real.py --
...
Uses stage_model() (same pattern) since /tmp/shapira is not guaranteed
pre-populated in this session.

washington IS in the v14 training corpus's county_target_encoding_map (real
trained rate 0.875, confirmed live from shapira-models/v14/2026-05-27-180308/
metrics.json, same value the a3eafa08 and 19313 sessions used) -- reused
directly, no fallback needed.

Live diagnosis this session (direct multi_county_auctions vs bid_decisions join,
after this same session's C/D/I fix via
scripts/gold_standard_shard3_washington_cd_i_20260826.py moved washington
6/10->9/10): exactly 5 washington case_numbers fail the deal_complete
definition -- 2026-TD-123/124/133/134/145, all auction_date 2026-08-25 (part of
the 15-row 2026-08-25 batch), all with zero existing bid_decisions row
(first-time fill). The other 10 rows of that same 15-row batch already had a
complete bid_decisions row on file from an earlier ingestion pass (VERIFIED via
direct query this session) -- so only these 5 are in scope, not all 15.
"""
...
COUNTY = "washington"
TARGET_CASE_NUMBERS = {
    "2026-TD-123", "2026-TD-124", "2026-TD-133", "2026-TD-134", "2026-TD-145",
}
NEED_FACTOR_KEYS = {"distress_location", "distress_property", "distress_owner", "cma_distressed", "cma_resale"}
COUNTY_TARGET_ENC = 0.875
PIPELINE_VERSION = "gold_standard_shard3_washington_shapira_v14_real_20260826"
