...
KNOWN_PARCEL_ZONES = {
    # 06820-010-091 already carries a correct parcel_zones row under
    # jurisdiction 1404 (Unincorporated Alachua County, code=R-1AA, sourced
    # from live ArcGIS FeatureServer) — do not add a second, Gainesville-
    # jurisdiction row for it; that duplicate previously orphaned the G join
    # (verified live 2026-07-20). Intentionally absent from this map.
    "02975-002-000": {
        # jurisdiction 973 (Alachua city) has no 'AG' code — real code is 'A'
        # (Agricultural District, verified far/pk1000-exempt in
...
def resolve_jurisdiction_id(jurisdiction_hint: str, jurisdictions: list[dict]) -> int | None:
    ...
    # Exact match first — "alachua" as a substring also matches
    # "Unincorporated Alachua County", which would silently misroute the
    # Alachua-city branch to the county jurisdiction (verified live
    # 2026-07-20: county has no RSF-1/A code, only R-1AA/R-1A, so the
    # misroute orphans the G join). Exact match disambiguates city vs county.
...
def main():
        ...
        if pid in KNOWN_PARCEL_ZONES:
            ...
            if arcgis_data:
                ...
                if "GAINESVILLE" in fulladdr:
                    # jurisdiction 915 (Gainesville) has no RSF-1 code — only
                    # 'SF' carries real zone_standards (far/parking/density
                    # all populated). A code that doesn't exist orphans the
                    # v_zoning_gold_standard_kpi_v3 join and regresses G
                    # (verified live 2026-07-20 — see broward I fix for the
                    # same failure mode).
    ...
    # get_existing_parcel_zones_alachua() above already pre-filters rows
    # already present, which is the idempotency guard for this script.
    n = rest_post("parcel_zones", insert_rows)
    log(f"Inserted {n} parcel_zones rows for alachua [VERIFIED]", "INFO", "VERIFIED")
