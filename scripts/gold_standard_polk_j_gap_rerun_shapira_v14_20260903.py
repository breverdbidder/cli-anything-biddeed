#!/usr/bin/env python3
"""GOLD STANDARD shard-5, county=polk, issue=19775 -- letter J (deal_complete) gap rerun.

CONTEXT: adapted verbatim (methodology) from
scripts/gold_standard_polk_j_generator_real.py (the prior polk J session that
took deal_complete 93.5% -> 100.0%, commit d2b946d5). Since that run, polk's
auctions_total grew (798 -> 1010, per this session's C/D/E/I fixes), so a fresh
GAP-ONLY rerun is needed: diff current in-scope multi_county_auctions against
current bid_decisions, process ONLY the newly-uncovered case_numbers.

Uses STABLE `order=id.asc` / `order=case_number.asc` explicit-cursor pagination
throughout (this session discovered unordered offset pagination against
multi_county_auctions silently under/over-counts rows while other shards write
concurrently -- verified three consecutive unordered runs of an identical query
returned 1010/710/412 for a stable 8676-row table; ordered pagination was
reproducibly stable at 1010 on repeat runs).

Real per-property Shapira V14 XGBoost inference (shapira_models.model_version=
'v14.0', AUC 0.7834, storage_bucket shapira-models), no county-level or
ml-score-bucket defaults, BLANK > WRONG for rows with no real value on file.
polk IS in the v14 training corpus's county_target_encoding_map (real trained
rate 0.7018022406234778, confirmed live in the prior polk J session from
shapira-models/v14/2026-05-27-180308/metrics.json) -- reused directly here,
no fallback needed (same constant, not re-fetched -- it is a fixed per-county
training artifact, not something that changes between fixes).

Usage: python3 scripts/gold_standard_polk_j_gap_rerun_shapira_v14_20260903.py
"""
import os, sys, json, math, re, time, urllib.request, urllib.error
...
PIPELINE_VERSION = "polk_j_shapira_v14_real_shard5_19775_gap_rerun"
...
def get_all(client, table, params, order_col="id"):
    """STABLE explicit-cursor pagination -- required against tables under
    concurrent write load (see module docstring)."""
    ...
    while True:
        ...
        for attempt in range(6):
            try:
                r = client.get(f"{SUPABASE_URL}/rest/v1/{table}", headers=HEADERS, params=p, timeout=90)
                r.raise_for_status()
                batch = r.json()
                break
...
def main():
    ...
    print(f"=== {COUNTY} (gap rerun, stable pagination) ===")
    auctions = get_all(client, "multi_county_auctions", {
        "select": "id,case_number,parcel_id,judgment_amount,opening_bid,market_value,assessed_value,"
    ...
    print(f"  raw fetched: {len(auctions)}")
    auctions = [a for a in auctions if a.get("case_number") and
                ((a.get("data_source") or "") != "propertyonion" or a.get("tier1_authoritative") is True)]
    print(f"  in-scope: {len(auctions)}")

    existing = get_all(client, "bid_decisions",
                        {"select": "id,case_number,parcel_id,arv,max_bid,ml_score,factors,pipeline_version",
                         "county_slug": f"eq.{COUNTY}"}, order_col="id")
    ...
    if parcel_ids:
        for i in range(0, len(parcel_ids), 200):
            ...
            parcels = get_all(client, "parcels", {"select": "parcel_uuid,parcel_id", "parcel_id": f"in.({','.join(chunk)})"}, order_col="parcel_uuid")
    ...
    if len(todo) > 0 and inserted == 0 and updated == 0 and skipped_no_value < len(todo):
