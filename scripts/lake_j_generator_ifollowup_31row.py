#!/usr/bin/env python3
"""Lake J (deal_complete) follow-up for the 32 rows that got real parcel_id
+ address (E-fix) and real assessed_value/lat/lon (I follow-up geo/value
backfill) in this session, but still lack a complete bid_decisions row.

Forked from scripts/shard2_lake_j_generator_real_v2.py (itself forked from
the santa_rosa/suwannee/broward_alachua real-XGBoost-inference lineage) --
county-agnostic Shapira V14 XGBoost methodology, same non-fabrication
contract: real ml_score from live model inference (shapira-models/v14 in
Supabase Storage, AUC 0.7834, verified against metrics.json before use),
real ARV from multi_county_auctions.assessed_value (all 32 confirmed
non-null live after the geo/value backfill step), real per-row haversine
distress-location proxy from real in-DB geocodes.

Scope: 31 of the 32 case_numbers (2025CA002238 already has a complete
bid_decisions row from a prior pass -- confirmed live before this script
ran -- so it is excluded from TARGET_CASE_NUMBERS to avoid touching an
already-passing row).

lake IS in the v14 training corpus's county_target_encoding_map (real rate
0.6406727828746177) -- confirmed live from the downloaded metrics.json,
matching the shard2_lake_j_generator_real_v2.py precedent exactly.
"""
...
REPAIR_TAG = "lake_j_generator_ifollowup_31row_shapira_v14_real"
...
TARGET_CASE_NUMBERS = [
    ...
    "2025CA002248", "2025CA002307", "2025CA002336",
