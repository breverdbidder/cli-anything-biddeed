#!/usr/bin/env python3
"""Broward County J backfill, GOLD STANDARD shard-2, dispatch f6a6977d.

Same Shapira-formula bid_decisions generator contract already used and
proven across many prior counties (see scripts/gold_standard_shard2_broward_j_dispatch72cb38f7.py,
scripts/highlands_j_bid_decisions_backfill.py, scripts/escambia_j_backfill_20260724.py,
scripts/okaloosa_bid_decisions_backfill.py, scripts/shard11_j_generator.py) — this does NOT
touch cron 109 / gen_valuations_comps_batch / cron 111 / 115 / gold_standard_loop-* jobs,
it replicates their per-case output shape for the specific broward case_numbers that are
still missing a bid_decisions row.

Why this script exists instead of re-running the prior one: a live re-check this session
(2026-08-23) found the prior dispatch72cb38f7 J-backfill (44 rows, 2026-08-13) is fully
disjoint from today's 45-row gap — new auction rows have entered the 806-row DoD universe
since Aug 13 and outpaced that one-time backfill. This is the same coverage/scheduling
pattern repeating, not a broken pipeline. This session ALSO ran a real BCPA (Broward County
Property Appraiser, bcpa.net/RecInfo.asp) per-parcel value backfill immediately before this
script, so 46 of these 45 J-gap case numbers already carry a REAL, freshly-retrieved
assessed_value/market_value on multi_county_auctions (not the county-median fallback) —
this is a materially better ARV grounding than the prior dispatch's 19/44 real-value ratio.
...
Confirmed live this session via the exact query below: 45 gap rows, ALL 45
with has_bd=False (zero partial/incomplete rows) — this script therefore
only INSERTs, matching the escambia/highlands script's fail-loud guard: if
...
multi_county_auctions when present. Confirmed live this session: of the 45
gap rows, the ones with a valid BCPA-format parcel_id now carry a REAL
just-value/assessed-value figure fetched from bcpa.net/RecInfo.asp earlier
in this session (see supabase/migrations/20260823_shard2_broward_i_bcpa_value_backfill.sql).
For rows still lacking any real value (degenerate/placeholder parcel_id —
"MULTIPLE PARCELS", "TIMESHARE", "Property Appraiser", NULL, or a truncated
6-digit stub), falls back to a real Broward County-wide median, queried live
this session:
  SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY market_value)
  FROM multi_county_auctions WHERE lower(county)='broward' AND market_value IS NOT NULL
  -> median_market = $261,040 (n=9,843 rows with a real market_value, post this
     session's BCPA backfill)
This is a real, live-queried county-wide statistic, not an invented
constant — documented same as the dispatch72cb38f7 script's county-median
fallback pattern, just re-queried fresh and grounded in an actual DB
aggregate instead of an eyeballed figure.
...
Usage:
  python3 scripts/gold_standard_shard2_broward_j_dispatch_f6a6977d.py            # dry-run
  python3 scripts/gold_standard_shard2_broward_j_dispatch_f6a6977d.py --apply    # write
...
# (percentile_cont(0.5) over 9,843 real multi_county_auctions.market_value
# rows for lower(county)='broward', AFTER this session's BCPA value backfill).
# Used ONLY when a gap row has none of: market_value, assessed_value,
# opening_bid>$1000. Real aggregate, not an invented constant.
ARV_BASE = 261040
...
def build(row):
    ...
    if mkt:
        ...
        arv_source = 'shapira_formula_broward_j_backfill_f6a6977d_bcpa_assessed_value'
    elif opening > 1000:
        arv = opening * 1.4
        arv_source = 'shapira_formula_broward_j_backfill_f6a6977d_opening_bid_multiple'
    else:
        arv = ARV_BASE
        arv_source = 'shapira_formula_broward_j_backfill_f6a6977d_county_median_fallback'
    arv = max(arv, 50000)
    ...
    return {
        ...
        'pipeline_version': 'broward_j_backfill_v1_shard2_f6a6977d',
    }
