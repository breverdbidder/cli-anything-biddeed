#!/usr/bin/env python3
"""
SHARD-20 County Status Verification
Check current A-J letter grades for charlotte, citrus, broward

Usage:
  python scripts/verify_shard20_status.py
"""
import os
import requests
import json
...
def identify_priority_targets(county_data):
    ...
    # Priority order based on briefing instructions
...
def main():
    print("🔍 SHARD-20 County Status Verification")
    print(f"Target counties: {', '.join(SHARD20_COUNTIES)}")
    ...
    # Generate reports
    print("\n" + "="*60)
    print("SHARD-20 COUNTY STATUS REPORT")
    print("="*60)
    ...
    print(f"\n" + "="*60)
    print("CRITERION-PARALLEL ACTION PLAN")
    print("="*60)
    
    print("\n🎯 **Priority Order (from briefing):**")
    print("1. **C/D ROOT CAUSE** - PropertyOnion coverage vs our matcher")
    print("2. **J GENERATOR** - build bid_decisions pipeline") 
    print("3. **B RECONCILIATION** - fix verified_outcomes anomalies")
    print("4. **G+I SUBSTRATE** - zoning districts + property cards")
    print("5. **E LINKAGE** - parcel_id via county GIS")
    print("6. **F AUTOMATION** - tier1 promotion (already automated)")
    
    print("\n📝 **Next Steps:**")
    print("1. Focus C/D: check PropertyOnion vs clerk/official records coverage")
    print("2. Build J: implement Shapira Formula pipeline for bid_decisions") 
    print("3. Debug B: reconcile verified_outcomes vs closed_sold counts")
