...
def main():
    ...
    # First: discover what fields the GIS layer has
    log("Probing GIS layer fields...", "GIS")
    probe = gis_query("1=1", "OBJECTID,dsp_strap,strap,owner_name,own_addr1,own_city")
    if not probe:
    ...
    if probe.get("error"):
        ...
        log("Trying alternate field names...", "GIS")
        probe2 = gis_query("1=1", "OBJECTID")
        if not probe2 or probe2.get("error"):
            log("Layer totally unreachable", "ERROR")
            sys.exit(1)
        features = probe2.get("features", [])
        log(f"Layer reachable, {len(features)} sample features with OBJECTID only")
    else:
    ...
    for target in TARGET_CASES:
        ...
        # Strategy 1: exact last-name LIKE search on owner_name
        where = f"UPPER(owner_name) LIKE '%{last_name}%'"
        data = gis_query(where, "OBJECTID,dsp_strap,strap,owner_name,own_addr1,own_city,cap_val,use_dscr")
        time.sleep(0.5)

        if data and not data.get("error") and data.get("features"):
            features = data["features"]
            log(f"  owner_name LIKE '%{last_name}%' -> {len(features)} features", "GIS")
            for f in features:
                a = f.get("attributes", {})
                gis_owner = (a.get("owner_name") or "").upper()
                if normalize_name(owner) in gis_owner or gis_owner in normalize_name(owner):
                    matched_feature = f
                    log(f"  MATCH: strap={a.get('dsp_strap')!r} owner={a.get('owner_name')!r}", "GIS")
                    break
                # Fuzzy: any significant word in common
                owner_words = set(normalize_name(owner).split())
                gis_words = set(gis_owner.split())
                if len(owner_words & gis_words) >= 2:
                    matched_feature = f
                    log(f"  FUZZY MATCH: strap={a.get('dsp_strap')!r} owner={a.get('owner_name')!r}", "GIS")
                    break
        elif data and data.get("error"):
            log(f"  owner_name query error: {data['error']}", "WARN")
            # Try alternate field: OWN_NAME
            where2 = f"UPPER(OWN_NAME) LIKE '%{last_name}%'"
            data2 = gis_query(where2, "OBJECTID,dsp_strap,OWN_NAME,OWN_ADDR1,CAP_VAL,USE_DSCR")
            time.sleep(0.5)
            if data2 and not data2.get("error") and data2.get("features"):
                features = data2["features"]
                log(f"  OWN_NAME LIKE '%{last_name}%' -> {len(features)} features", "GIS")
                for f in features:
                    a = f.get("attributes", {})
                    gis_owner = (a.get("OWN_NAME") or "").upper()
                    owner_words = set(normalize_name(owner).split())
                    gis_words = set(gis_owner.split())
                    if len(owner_words & gis_words) >= 2:
                        matched_feature = f
                        log(f"  FUZZY MATCH OWN_NAME: strap={a.get('dsp_strap')!r} owner={a.get('OWN_NAME')!r}", "GIS")
                        break
