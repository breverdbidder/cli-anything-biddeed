#!/usr/bin/env python3
"""SHARD-11 RUN-3534, escambia C/D/E/I backfill.
...
logic already shipped in scripts/shard9_flagler_cd_ajax_harvest.py (same pattern
used for flagler/bay/martin/alachua/lake), applied to escambia.

Platform: escambia.realforeclose.com (foreclosure) + escambia.realtaxdeed.com
(tax_deed), subdomain='escambia' for both.

For each distinct (sale_type, auction_date) present in escambia's
multi_county_auctions rows that is NOT already tier1 matched_clean:
  ...
  3. PATCH parity_status='matched_clean',
     parity_source='tier1:shard11_run3534_escambia_ajax_harvest:<sale_type>:<date>'
  4. opportunistically backfill parcel_id/property_address/assessed_value when
     missing on the MCA row (idempotent — only fills NULLs), which also lifts I
     toward its ceiling (I<=E)

Direct DB (psycopg2/pooler) not used — PostgREST only, consistent with prior
shard sessions (password auth confirmed stale).

Usage: python3 scripts/shard11_run3534_escambia_cd_harvest.py '<json targets>'
  targets: [{"county":"escambia","sale_type":"tax_deed","auction_date":"2026-08-05"}, ...]
"""
import os
import sys
import json
...
def main():
    if not TARGETS:
        print("usage: shard11_run3534_escambia_cd_harvest.py '<json targets>'")
        sys.exit(1)
    ...
    for t in TARGETS:
        ...
        try:
            parity, parcel, card = match_and_fix(
                county, items, f"tier1:shard11_run3534_escambia_ajax_harvest:{sale_type}:{ad}")
        except Exception as e:
