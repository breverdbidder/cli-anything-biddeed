#!/usr/bin/env python3
"""Santa Rosa real J generator — county-agnostic Shapira V14 XGBoost methodology, forked from
scripts/shard8_run6080_suwannee_j_generator_real.py (itself forked from
scripts/gold_standard_shard9_broward_alachua_j_generator_real.py, the non-fabricated pattern).

Ghost-success found live this session (dispatch cc621572-35e9-41fd-a901-e5719416b834): ALL 104
existing santa_rosa bid_decisions rows carry a constant/bucketed ml_score regardless of
pipeline_version tag -- 0.45 for the 77 rows tagged 'v14.0_heuristic'/'shard6_loop581_j_fix'
(a misleadingly-named tag; there is no real v14 inference in that code path) and a 2-value
bucket (0.38/0.75, keyed only on whether max_bid>0) for the 27 rows tagged 'shard9_j_gen_v1'
(scripts/shard9_j_generator.py, which also hardcodes a single county-wide ARV constant of
$295,000 as its no-real-value fallback). This generator computes a genuine per-property
ml_score via real XGBoost inference against the live shapira_models v14.0 artifact (AUC
0.7834), and skips (does not fabricate) any auction with no real assessed/market value on file.

county_target_enc: santa_rosa IS one of the 45 counties in the v14 training corpus's
county_target_encoding_map (real trained rate 0.70720), so this uses that real value directly
rather than the cross-county-mean fallback suwannee needed.

Location-distress reference point: median lat/lon of santa_rosa's OWN 89 real-geocoded
auction rows (30.630792, -87.0244), not an externally-asserted "county seat" coordinate --
this keeps the signal traceable to VERIFIED in-DB data rather than an unverified external claim.
"""
...
# v1 of this real generator is included here too: an adversarial refuter caught it
# silently collapsing distress_owner to a constant 0.35 (owner_name is null on every
# santa_rosa auction row; v1 never fell back to fl_parcels.own_name), so v1's output
# needs the same forced-reprocess treatment as the original fabricated tags.
FABRICATED_PIPELINE_VERSIONS = {
    "v14.0_heuristic", "shard9_j_gen_v1", "shard6_loop581_j_fix",
    "santa_rosa_j_generator_run6080_shapira_v14_real",
}
REPAIR_TAG = "santa_rosa_j_generator_run6080_shapira_v14_real_v2"

# Real per-county trained target-encoding rate from shapira_models v14 metrics.json
# (santa_rosa IS in the 45-county training corpus, no fallback needed).
COUNTY_TARGET_ENC = 0.7071984435797666

# Real centroid of santa_rosa's own 89 geocoded auction rows (median lat/lon), computed
# live this session -- not an externally-asserted county-seat coordinate.
REF_LAT, REF_LON = 30.630791697653, -87.0244
...
def main():
    ...
    # owner_name is NULL on every santa_rosa multi_county_auctions row (verified live
    # this session) -- an earlier version of this script silently fell back to a
    # constant distress_owner=0.35 for all 89 rows, which an adversarial refuter caught
    # as the same degenerate-constant fabrication pattern relocated from ml_score.
    # fl_parcels.own_name carries the real appraiser-of-record owner name for the 87
    # santa_rosa rows with a linked parcel_id -- use that as a genuine per-row source.
    ...
    def needs_repair(existing_row):
        # Every pipeline_version tag on file for santa_rosa is confirmed-fabricated
        # (constant/bucketed ml_score regardless of tag) -- force full reprocess.
    ...
    print(f"to_process={len(todo)} (all existing rows are fabricated per pipeline_version audit; force full reprocess)")
    ...
    # Purge orphaned fabricated rows: bid_decisions rows tagged with any of the
    # confirmed-fabricated pipeline_version values whose case_number is NOT among the
    # county's current live auctions (so the reprocess loop above never touched them).
    # An adversarial refuter caught these as stale, untouched, fabricated data left
    # behind by the fix -- purge them rather than leave dead fabricated rows on file.
