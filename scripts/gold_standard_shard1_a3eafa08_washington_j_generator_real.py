#!/usr/bin/env python3
"""
Gold Standard shard-1 (dispatch a3eafa08) — REAL J generator for washington,
forked verbatim (methodology) from scripts/gold_standard_shard9_osceola_run255f_j_generator_real.py,
the non-fabricated pattern: real per-property Shapira V14 XGBoost inference
(shapira_models.model_version='v14.0', AUC 0.7834), no county-level or
ml-score-bucket defaults, BLANK > WRONG for rows with no real value on file.

washington IS in the v14 training corpus's county_target_encoding_map (real
trained rate 0.875, confirmed live from
shapira-models/v14/2026-05-27-180308/metrics.json) -- uses that real value
directly, no cross-county-mean fallback needed (that fallback, 0.6373745865476843,
is documented in scripts/shard7_wakulla_j_generator_real.py for counties that
are NOT in the 45-county corpus; washington is not one of those cases).

Live diagnosis this session (SQL against pencil_dod_evaluate_county's own
join): exactly 11 washington case_numbers fail the deal_complete definition
(2026-TD-065/067/068/070/084/085/086/087/088/089/090, all auction_date
2026-08-11), all 11 with zero existing bid_decisions row (not a stale/bucketed
row to purge -- first-time fill). All 11 are $2900-assessed vacant
residential lots/rights-of-way in Chipley, FL (property_address values like
"NORTON CT, CHIPLEY, FL- 32428") with market_value, judgment_amount,
beds/baths/sqft, and prior_sale_price all NULL -- no parcel_valuations
comps_cma_bulk row exists for any of the 11 parcel_ids either. The only real
per-property value on file is assessed_value=2900.00 for all 11, which is
the same real_arv() fallback tier (multi_county_auctions.assessed_value) the
osceola template already treats as legitimate, non-fabricated ARV input --
so all 11 get real, non-fabricated rows here rather than being left BLANK.
"""
...
COUNTY = "washington"
TARGET_CASE_NUMBERS = {
    "2026-TD-065", "2026-TD-067", "2026-TD-068", "2026-TD-070", "2026-TD-084",
    "2026-TD-085", "2026-TD-086", "2026-TD-087", "2026-TD-088", "2026-TD-089", "2026-TD-090",
}
...
# (washington IS in the 45-county training corpus, no fallback needed).
COUNTY_TARGET_ENC = 0.875
PIPELINE_VERSION = "shard1_a3eafa08_washington_shapira_v14_real"
...
def main():
    ...
    print(f"  target_case_numbers={len(TARGET_CASE_NUMBERS)} matched_in_auctions={len(auctions)}")
    missing = TARGET_CASE_NUMBERS - {a["case_number"] for a in auctions}
    if missing:
    ...
    if collision_parcels:
        print(f"  WARNING: {len(collision_parcels)} parcel_id values map to multiple distinct addresses "
              f"in washington's target set -- excluded from comps join.")
