#!/usr/bin/env python3
"""SHARD-9, flagler C/D/E/I backfill.
...
logic from scripts/shard14_martin_bay_alachua_lake_cd_e_i_fix.py (same proven
pattern already shipped for bay/martin/alachua/lake), applied to flagler.

For each distinct (sale_type, auction_date) present in flagler's
multi_county_auctions rows:
  ...
  3. PATCH parity_status='matched_clean', parity_source='tier1:shard9_flagler_ajax_harvest:<sale_type>:<date>'
...
Usage: python3 scripts/shard9_flagler_cd_ajax_harvest.py '<json targets>'
  targets: [{"county":"flagler","sale_type":"foreclosure","auction_date":"2026-03-13"}, ...]
"""
import os
import sys
import json
    ...
    for t in TARGETS:
        ...
        try:
            parity, parcel, card = match_and_fix(
                county, items, f"tier1:shard9_flagler_ajax_harvest:{sale_type}:{ad}")
        except Exception as e:
