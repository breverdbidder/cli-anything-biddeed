...
def posthog_funnel(api_key, project_id, date_from, date_to):
    """Query PostHog funnel API for the acquisition funnel."""
    funnel_payload = {
        "insight": "FUNNELS",
...
def posthog_events_count(api_key, project_id, event_name, date_from, date_to):
    """Get total count of an event in the date range."""
    params = f"event={urllib.parse.quote(event_name)}&date_from={date_from}&date_to={date_to}"
    req = urllib.request.Request(
        f"{POSTHOG_HOST}/api/projects/{project_id}/events/?{params}&limit=1",
