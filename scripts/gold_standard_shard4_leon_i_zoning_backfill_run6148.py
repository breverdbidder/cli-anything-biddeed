...
Root cause (verified live 2026-07-24 via SQL): leon auctions_total grew 165->189
since the 2026-07-18 I-fix (dispatch 7066f088). 31 of the 189 rows fail
card_complete; 30 of those fail solely because parcel_id is not linked into
parcel_zones with a zone_code (v_zoning_gold_standard_card join) -- these are
auctions added to the calendar after the prior fix and never zoning-enriched.

Technique: the TLC_OverlayZoning_D_WM ArcGIS layer (intervector.leoncountyfl.gov)
has NO parcel-id attribute field (verified live: PARCELID where-clause returns
HTTP 400 "Failed to execute query" -- the field does not exist on this layer,
per /MapServer/0?f=json metadata). The correct join is spatial point-in-polygon
using the row's own lat/lon against the zoning polygon, returning ZONING +
JURISDICTION (verified live: (30.455429909805,-84.303726204421) -> ZONING=RP-2,
JURISDICTION=City). Rows missing lat/lon are geocoded first via the free US
Census Bureau geocoder (Public_AR_Current benchmark), same technique already
proven in scripts/gold_standard_shard11_leon_i_geocode.py.
...
def main():
    print("=== leon I zoning backfill | dispatch=0fc2eae2 | run6148 ===")
    ...
    print(f"Total leon in-scope rows: {len(rows)}")
    ...
    for row in gap:
        ...
        try:
            rest_post("parcel_zones", {
                ...
                "source": f"tlcgis_intervector_zoning_layer_spatial:shard4-run6148:{DISPATCH_ID[:8]}",
    ...
    audit = {
        ...
        "letter": "I",
        "claim": (
            f"leon I zoning backfill (run6148): gap={len(gap)} zoned={zoned} geocoded={geocoded} "
            f"via TLC GIS spatial point-in-polygon (intervector.leoncountyfl.gov, verified live "
            f"field metadata has no PARCELID -- spatial join is the only valid technique on this "
            f"layer) + Census geocoder fallback for missing coords. metric {before['I']['metric']} -> {after['I']['metric']}."
        ),
