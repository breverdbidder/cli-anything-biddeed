...
def seed_broward_td() -> int:
    """DO NOT re-enable synthetic seeding — see history below.

    This function previously inserted a fabricated tax_deed row
    (case_number='2024-TDD-BROWARD-001', data_source='synthetic_seed') to
    flip letter A from FAIL to PASS. That row was already inserted, caught
    as ghost-success, and DELETED by a prior session on 2026-07-02
    (migrations/20260702_shard3_franklin_broward_synthetic_quarantine.sql).
    This dispatch (20a33672) reinserted the identical row a second time; it
    was caught again by ULTRALOOP adversarial verification and reverted
    live 2026-07-20. Real td (tax_deed) coverage for broward is genuinely
    zero because broward.realtaxdeed.com returns HTTP 403 to scrapers — the
    fix is a real authenticated scraper (see campaign playbook A), not a
    placeholder row. This function now only reports the honest count.
    """
    td_rows = rest_get(
        "multi_county_auctions?county=eq.broward&sale_type=eq.tax_deed"
        "&data_source=neq.propertyonion&select=count"
    )
    td_count = int(td_rows[0].get("count", 0)) if td_rows else 0
    log(f"Broward real (non-propertyonion) td count: {td_count} — "
        f"CONFIRMED DEAD END, not seeding a placeholder", "INFO", "VERIFIED")
    return td_count
...
def fix_alachua_j() -> int:
    ...
    # Require a linked parcel AND at least one real per-property value signal.
    # Without this guard, rows with no parcel_id/value data (e.g. future
    # auctions not yet enriched) fall through calc()'s DEFAULT_ARV fallback
    # for every row, producing byte-identical "deal thesis" numbers across
    # unrelated properties — a fabricated ghost-success caught live by
    # ULTRALOOP adversarial verification 2026-07-20 (4 alachua rows reverted).
    new_auctions = [
        a for a in auctions
        if a.get("case_number")
        and a["case_number"] not in existing_cases
        and a.get("parcel_id")
        and (a.get("assessed_value") or a.get("market_value") or a.get("opening_bid"))
    ]
    log(f"Alachua gap auctions (no bid_decisions, real value signal present): {len(new_auctions)}", "INFO", "VERIFIED")
    if not new_auctions:
        return 0
    ...
    # bid_decisions has no unique constraint on (case_number, county_slug) —
    # only PK on id — so on_conflict is unusable. existing_cases above is
    # the idempotency guard.
    n = rest_post("bid_decisions", rows)
