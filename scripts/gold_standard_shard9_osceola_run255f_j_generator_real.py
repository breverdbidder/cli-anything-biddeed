#!/usr/bin/env python3
"""
Gold Standard shard-9 (dispatch 255f0be0) — REAL J generator for osceola,
forked verbatim (methodology) from scripts/gold_standard_shard9_broward_alachua_j_generator_real.py,
...
osceola IS in the v14 training corpus's county_target_encoding_map (real
trained rate 0.5563829787234043, confirmed live from
...
join): exactly 8 osceola case_numbers fail the deal_complete definition, all
8 with zero existing bid_decisions row (not a stale/bucketed row to purge --
first-time fill).
"""
...
# (osceola IS in the 45-county training corpus, no fallback needed).
COUNTY_TARGET_ENC = 0.5563829787234043
PIPELINE_VERSION = "shard9_255f0be0_osceola_shapira_v14_real"
...
def main():
    ...
    if collision_parcels:
        print(f"  WARNING: {len(collision_parcels)} parcel_id values map to multiple distinct addresses "
              f"in osceola's auction set -- excluded from comps join.")
