#!/usr/bin/env python3
"""
Alachua-only J fix (fork of scripts/gold_standard_shard9_broward_alachua_j_generator_real.py).

Root cause (per prior diagnosis, VERIFIED live 2026-07-31): 11 alachua auctions
have NO bid_decisions row at all (coverage lag -- ingested/re-swept after the
last J-generator firing). Of those 11, 9 have assessed_value AND market_value
both null, so the parent script's real_arv() fallback chain (comps ->
assessed_value -> market_value) gives up on them even on a fresh rerun,
landing them in skipped_no_real_value instead of being written.

Fix: extend real_arv() with a 4th tier -- judgment_amount, then opening_bid --
both real, per-property, court-filed/clerk-published figures already present
on the auction row (NOT a fabricated constant, NOT a county-level default).
This mirrors the shard14/lee generator's use of a documented last-resort
signal, but stays honest: no `opening_bid * 1.4` multiplier here, just the
real dollar figure itself, tagged with its own arv_source.

Case 01 2025 CA 002643 has zero real value inputs (no comps, no assessed/
market, no judgment_amount, no opening_bid) -- per BLANK > WRONG this row is
deliberately left with skipped_no_real_value and NOT force-written with an
invented value. That caps this fix at 10/11 -> 57/58 = 98.3% (>= 95% bar).

Scope: alachua ONLY. Does not touch broward (out of scope per diagnosis note
7, even though the same fallback gap likely exists there too).

Idempotent: reuses the parent script's bid_decision_complete() idempotency
check -- only inserts/updates rows currently missing arv/max_bid/ml_score or
any of the 5 canon factor keys. Never overwrites a row that already has real
data from a different arv_source.
"""
...
COUNTIES = ["alachua"]  # scope-limited fork -- broward intentionally excluded
NEED_FACTOR_KEYS = {"distress_location", "distress_property", "distress_owner", "cma_distressed", "cma_resale"}
# Real per-county base rate from shapira_models.metrics.county_target_encoding_map
# (v14/2026-05-27-180308/metrics.json), NOT a fabricated constant.
COUNTY_TARGET_ENC = {"alachua": 0.5655502392344498}
GLOBAL_TARGET_ENC = 0.60
PIPELINE_VERSION = "alachua_J_fix_judgment_opening_fallback_v1"
