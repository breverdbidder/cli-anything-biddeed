#!/usr/bin/env python3
"""GOLD STANDARD shard4, dispatch d88f924a-fe76-4639-84ea-e585fdbf62c3, county=leon.

Forked verbatim from scripts/gold_standard_shard11_leon_cd_i_ajax_harvest.py (the
prior leon C/D/I session), itself forked from shard11_run3534_duval_cd_harvest.py.
Only the parity_source run label changed to reflect this dispatch id; harvesting
logic is untouched.

C/D backfill for leon (162 total auction rows, 9 with parity_status IS NULL:
7 foreclosure rows across 4 auction dates in Jul 2026, 2 tax_deed rows sharing
one auction date in Aug 2026 with short-format case numbers 14-0367/16-0785).

Forked verbatim from scripts/shard11_run3534_duval_cd_harvest.py (itself forked
from scripts/shard9_flagler_cd_ajax_harvest.py), which reuses the proven AJAX
RealAuction/RealTaxDeed harvester at scripts/shard2_run2450_ajax_realforeclose_harvest.py
and the match_and_fix() pattern from
scripts/shard14_martin_bay_alachua_lake_cd_e_i_fix.py.

For each distinct (sale_type, auction_date) present in leon's multi_county_auctions
rows with parity_status IS NULL:
  1. harvest the live RealAuction/RealTaxDeed calendar for that date via the AJAX
     endpoint (leon.realforeclose.com / leon.realtaxdeed.com) -- both verified
     live 200 OK in this session before this script was written
  2. exact-match by normalized case_number (short tax-deed case numbers like
     14-0367 normalize identically on both sides -- non-alphanumeric chars stripped)
  3. PATCH parity_status='matched_clean',
     parity_source='tier1:shard4_run_d88f924a-fe76-4639-84ea-e585fdbf62c3_ajax_harvest:<sale_type>:<date>'
  4. opportunistically backfill parcel_id/property_address/assessed_value when
     missing on the MCA row (idempotent -- only fills NULLs; contributes to I
     for the same rows, though I also needs lat/lng which this harvester does
     not supply)

Direct DB (psycopg2/pooler) not used -- PostgREST only, consistent with prior
shard sessions (password auth confirmed stale this session per dispatch brief).

Fails loud: if a date's calendar returns >0 parsed items but 0 rows get
parity-promoted, that is logged explicitly (not swallowed) -- though per the
harness contract this is a per-date print, not a raise, matching the proven
shard11/shard14 precedent scripts (a single bad date should not abort the
whole 9-row run).

Usage: python3 scripts/gold_standard_shard11_leon_cd_i_ajax_harvest.py '<json targets>'
  targets: [{"county":"leon","sale_type":"foreclosure","auction_date":"2026-07-10"}, ...]
"""
import os
import sys
import json
...
def match_and_fix(county, items, parity_source_label, target_case_numbers=None):
    ...
    for row in mca_rows:
        ...
        if cn not in by_norm:
            continue
        # Only touch the 9 target rows for this dispatch -- do not silently
        # promote unrelated already-matched leon rows.
...
def main():
    if not TARGETS:
        print("usage: gold_standard_shard11_leon_cd_i_ajax_harvest.py '<json targets>'")
        sys.exit(1)
    ...
    for t in TARGETS:
        ...
        try:
            parity, parcel, card = match_and_fix(
                county, items, f"tier1:shard4_run_d88f924a-fe76-4639-84ea-e585fdbf62c3_ajax_harvest:{sale_type}:{ad}",
