#!/usr/bin/env python3
"""Lake J (deal_complete) follow-up, 2nd pass, single row: case 2025CA002152.

This case was NOT part of the E-fix's 32-row scope -- it already had a real
parcel_id/address/assessed_value ($517,702) on file before this session --
but it independently fails J because its bid_decisions row is an empty
stub (arv/max_bid/ml_score/factors all NULL). Same generator, same model
artifact, same non-fabrication contract, single-case scope.

Forked from scripts/lake_j_generator_ifollowup_31row.py (itself forked from
scripts/shard2_lake_j_generator_real_v2.py).
"""
...
REPAIR_TAG = "lake_j_generator_ifollowup_1row_002152_shapira_v14_real"
