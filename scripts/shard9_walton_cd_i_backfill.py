    ...
    try:
        result = arcgis_query(
            ENERG0V_PARCELS,
            {
                ...
                "outFields": "PARCELNO,OWNNAME,SITEADDR",
                "returnGeometry": "true",
        ...
        return {
            ...
            "site_address": (attrs.get("SITEADDR") or "").strip() or None,
            "owner_name": (attrs.get("OWNNAME") or "").strip() or None,
        }
    except Exception as e:
...
def ensure_zoning_district(jur_id: int, zone_code: str) -> None:
    ...
    existing = sb_get(
        "zoning_districts",
        {"select": "id", "jurisdiction_id": f"eq.{jur_id}", "code": f"eq.{urllib.parse.quote(zone_code)}", "limit": "1"},
    )
    if existing:
        return
    CATEGORY_MAP = {
    ...
    sb_post(
        "zoning_districts",
        {
            ...
            "data_source": f"walton_enerGov_arcgis_s9_{DISPATCH_ID[:8]}",
        },
