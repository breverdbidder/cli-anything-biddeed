#!/usr/bin/env python3
"""GOLD STANDARD shard-9 run6046 -- bay B/F fix (RealTaxDeed Auction Results
Report), tax_deed lane.

Root cause (confirmed live 2026-07-23 via pencil_dod_evaluate_county('bay')):
  bay has 75 tax_deed rows in multi_county_auctions. 17 of them are
  past-dated (auction_date < today) but only 2 carry auction_status=
  'concluded' -- the other 15 were simply never closed out with a sold
  outcome. tax_deed_outcomes for bay was 0 rows, so B (verified_outcomes/
  closed_sold) and F (tier1_sold/closed_sold) both evaluate closed_sold=0
  -> null metric, pass=false.

  This targets exactly that gap using the SAME proven technique already
  used for sarasota (scripts/gold_standard_shard6_run5361_sarasota_bcdf_realtaxdeed_results.py),
  hendry, and santa_rosa: the authenticated bay.realtaxdeed.com "Auction
  Results Report" (report_id=18 -- CONFIRMED live for bay by direct probe
  2026-07-23, same report_id as sarasota/hendry/santa_rosa; RealTaxDeed/
  RealForeclose share the same underlying RealAuction platform, same
  login/notice-drain/report flow).

  This is the Bay Clerk's OWN post-sale ledger (ar.winning_bid written by
  the Clerk/RealAuction backend after each auction closes) -- an
  INDEPENDENT source from our own pre-sale calendar-sweep scraper
  (data_source='calendar_sweep_mca_v3' rows in multi_county_auctions with
  sold_amount still null).

  GRID LAYOUT CONFIRMED DIFFERENT FROM SARASOTA (empirically probed live
  2026-07-23, 301 rows across 4 pages): bay's grid returns 16 cells same as
  sarasota/hendry's COLS layout, BUT bay's case_number cell[1] is PLAIN TEXT
  (e.g. '2026-3120TD'), NOT an HTML anchor like sarasota's
  '<a href="...">2026 TD 000188</a>'. The shared extract_case_number()
  fallback path (strip any HTML tags, which is a no-op on plain text) already
  handles this correctly -- verified against a live sample row. cert_or_deed_no
  cell[14] is a bare digit string ('3120'), auction_status cell[13] values
  observed live: 'Sold' (298/301) and 'Cancelled' (3/301) -- no 'Redeemed'
  seen in this pull, but the same HONESTY GUARD (only trust rows whose OWN
  auction_status says 'Sold') applies regardless.
...
'tier1_realtaxdeed_results_report:bay:gold_standard_shard9_run6046' and
sets parity_status/parity_source/tier1_sold_amount/tier1_authoritative on
the matching multi_county_auctions row so B and F move together.

NOTE: a parallel agent runs the equivalent script against
bay.realforeclose.com (foreclosure lane, sale_type=foreclosure) at the same
time. This script only ever touches sale_type=eq.tax_deed rows -- no row
overlap.

Usage:
  python3 scripts/shard9_run6046_bay_bf_realtaxdeed_results.py
  python3 scripts/shard9_run6046_bay_bf_realtaxdeed_results.py --dry-run
"""
...
DATA_SOURCE_TAG = "tier1_realtaxdeed_results_report:bay:gold_standard_shard9_run6046"
PARITY_SOURCE_TAG = "tier1_realtaxdeed_live:bay:gold_standard_shard9_run6046"
...
# bay's grid returns 16 cells, same column layout as sarasota/hendry, but
# case_number cell[1] is PLAIN TEXT (no HTML anchor) -- confirmed live
# 2026-07-23 across 301 sample rows (0 of 301 had an <a> tag in cell[1]).
# The shared extract_case_number() fallback (strip HTML tags, no-op on
# plain text) already handles this correctly.
...
def main():
    log("=== GOLD STANDARD SHARD-9 RUN6046 BAY B/F FIX (RealTaxDeed Auction Results Report) ===")
    ...
    # Wide range covering all bay tax_deed auction_date values we have on file.
    ...
    if not parsed:
        log("0 rows parsed from RealTaxDeed results report for bay -- "
    ...
    log(f"Fetched {len(mca_rows)} bay tax_deed multi_county_auctions rows", "VERIFIED")
    ...
    log(f"Matched {len(matched)} bay tax_deed rows to RealTaxDeed results with "
    ...
    if not_in_report:
        log(f"case_numbers not found in RealTaxDeed report: {not_in_report}", "VERIFIED")
    ...
    for row, rr in matched:
        if DRY_RUN:
            ...
            # bay.realtaxdeed.com Auction Results Report (report_id=18), the
            # Clerk's own post-sale ledger -- an independent, verified
    ...
    print("SELECT county, sold_amount_source, COUNT(*) FROM multi_county_auctions "
          "WHERE county='bay' AND sale_type='tax_deed' AND sold_amount IS NOT NULL "
