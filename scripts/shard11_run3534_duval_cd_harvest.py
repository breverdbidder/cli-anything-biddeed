#!/usr/bin/env python3
"""SHARD-11 RUN-3534, duval C backfill (D/E/F/I/J/G/A/H already pass — do not touch).

Forked verbatim from scripts/shard9_flagler_cd_ajax_harvest.py (same proven
pattern already shipped for flagler/bay/martin/alachua/lake), applied to duval.
Reuses the AJAX RealAuction/RealTaxDeed harvester from
scripts/shard2_run2450_ajax_realforeclose_harvest.py.

For each distinct (sale_type, auction_date) present in duval's
multi_county_auctions rows that are NOT already matched_clean with a tier1
parity_source:
  1. harvest the live RealAuction/RealTaxDeed calendar for that date via the AJAX
     endpoint (duval.realforeclose.com / duval.realtaxdeed.com)
  2. exact-match by normalized case_number
  3. PATCH parity_status='matched_clean',
     parity_source='tier1:shard11_run3534_duval_ajax_harvest:<sale_type>:<date>'
  4. opportunistically backfill parcel_id/property_address/assessed_value when
     missing on the MCA row (idempotent — only fills NULLs; E/I already pass at
     100%/96.1% so this is a no-op safety net, not the goal of this run)

Direct DB (psycopg2/pooler) not used — PostgREST only, consistent with prior
shard sessions (password auth confirmed stale).

Usage: python3 scripts/shard11_run3534_duval_cd_harvest.py '<json targets>'
  targets: [{"county":"duval","sale_type":"foreclosure","auction_date":"2026-03-13"}, ...]
"""
import os
import sys
import json
...
def main():
    if not TARGETS:
        print("usage: shard11_run3534_duval_cd_harvest.py '<json targets>'")
        sys.exit(1)
    ...
    for t in TARGETS:
        ...
        try:
            parity, parcel, card = match_and_fix(
                county, items, f"tier1:shard11_run3534_duval_ajax_harvest:{sale_type}:{ad}")
        except Exception as e:
