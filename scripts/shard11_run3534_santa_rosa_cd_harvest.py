#!/usr/bin/env python3
"""SHARD-11 run3534, santa_rosa C/D/E/I backfill.

Fork of scripts/shard9_flagler_cd_ajax_harvest.py (proven AJAX RealAuction/
RealTaxDeed harvester + exact-match/patch pattern), applied to santa_rosa.

NAMING TRAP handled here: santa_rosa's slug in multi_county_auctions is
'santa_rosa' (underscore) but the live RealAuction/RealTaxDeed subdomain is
'santarosa' (no underscore). Unlike the flagler original (where
subdomain == county == db slug), this fork takes an explicit `subdomain` key
per target and uses it ONLY for the harvest URL, while all DB reads/writes
still filter/write county='santa_rosa'.

For each distinct (sale_type, auction_date) present in santa_rosa's
multi_county_auctions rows:
  1. harvest the live RealAuction/RealTaxDeed calendar for that date via the AJAX
     endpoint (subdomain='santarosa')
  2. exact-match by normalized case_number
  3. PATCH parity_status='matched_clean', parity_source='tier1:shard11_run3534_santa_rosa_ajax_harvest:<sale_type>:<date>'
...
Usage: python3 scripts/shard11_run3534_santa_rosa_cd_harvest.py '<json targets>'
  targets: [{"county":"santa_rosa","subdomain":"santarosa","sale_type":"foreclosure","auction_date":"2026-07-15"}, ...]
"""
import os
import sys
import json
    ...
    for t in TARGETS:
        ...
        try:
            parity, parcel, card = match_and_fix(
                county, items, f"tier1:shard11_run3534_santa_rosa_ajax_harvest:{sale_type}:{ad}")
        except Exception as e:
