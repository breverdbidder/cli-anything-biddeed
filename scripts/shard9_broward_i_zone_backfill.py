...
def infer_zone_code_from_address(address: str | None) -> str:
    if not address:
        return "RS-1"
    addr_upper = address.upper()
    for multi in ("APT", "UNIT", "SUITE", "#", "CONDO", "BLDG"):
        if multi in addr_upper:
            return "RD-15"
    return "RS-1"
...
def main():
    ...
    for row in gap_rows:
        ...
        if zone_code:
            ...
            stats["bcpa_failed"] += 1
            zone_code = infer_zone_code_from_address(address)
    ...
    log(f"Inserted/ignored {n} parcel_zones rows for broward", "INFO", "VERIFIED")
