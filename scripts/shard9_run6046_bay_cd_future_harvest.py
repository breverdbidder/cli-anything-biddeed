#!/usr/bin/env python3
"""SHARD-9 (bay), GOLD STANDARD dispatch 503717c8-e819-470c-b363-6f20c13160e9, loop run 6046.

C/D/E/I backfill for bay. Reuses the proven AJAX RealAuction/RealTaxDeed harvester
from scripts/shard2_run2450_ajax_realforeclose_harvest.py (the same mechanism
shard8/shard9-run3059/shard14 used) to pull the live calendar per
(county, sale_type, auction_date), then:
  1. exact case_number match -> parity_status='matched_clean',
     parity_source='tier1:shard9_run6046_bay_ajax_harvest:<sale_type>:<date>' (C/D)
...
shard13/shard14 findings. All reads/writes go through PostgREST.
...
Usage: python3 scripts/shard9_run6046_bay_cd_future_harvest.py '<json targets>'
"""
import os
import sys
import json
...
def main():
    if not TARGETS:
        print("usage: shard9_run6046_bay_cd_future_harvest.py '<json targets>'")
        sys.exit(1)
    ...
    for t in TARGETS:
        ...
        try:
            parity, parcel, card = match_and_fix(
                county, items, f"tier1:shard9_run6046_bay_ajax_harvest:{sale_type}:{ad}")
        except Exception as e:
