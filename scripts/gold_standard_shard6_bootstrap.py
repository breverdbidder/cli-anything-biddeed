#!/usr/bin/env python3
"""
GOLD STANDARD SHARD-6 County Bootstrap: highlands, sumter, jackson, calhoun, liberty
Ensures our assigned counties have baseline data ingested before working on Letters

Usage:
  python scripts/gold_standard_shard6_bootstrap.py
  python scripts/gold_standard_shard6_bootstrap.py --county highlands
"""
import os
import sys
import subprocess
...
# SHARD-6 counties from fl_counties_manifest.yml (slugs need to be resolved in DB)
TARGET_COUNTIES = [
    {'name': 'Highlands', 'co_no': 38, 'slug': 'highlands'},
    {'name': 'Sumter', 'co_no': 70, 'slug': 'sumter'}, 
    {'name': 'Jackson', 'co_no': 42, 'slug': 'jackson'},
    {'name': 'Calhoun', 'co_no': 17, 'slug': 'calhoun'},
    {'name': 'Liberty', 'co_no': 49, 'slug': 'liberty'}
]
...
def main():
    parser = argparse.ArgumentParser(description='Gold Standard SHARD-6 County Bootstrap')
    parser.add_argument('--county', choices=['highlands', 'sumter', 'jackson', 'calhoun', 'liberty'], 
                       help='Single county to bootstrap')
    args = parser.parse_args()
    
    print("=" * 60)
    print("GOLD STANDARD SHARD-6 County Bootstrap")
    if args.county:
        print(f"Target County: {args.county}")
    else:
        print("Target Counties: highlands, sumter, jackson, calhoun, liberty")
    print("=" * 60)
    ...
    counties_to_check = TARGET_COUNTIES
    if args.county:
        counties_to_check = [c for c in TARGET_COUNTIES if c['slug'] == args.county]
    ...
    if not counties_to_ingest:
        ...
        print("\nNext steps:")
        print("  1. Run verification: python scripts/verify_shard6_status.py")
        print("  2. Focus on counties with partial progress for highest impact")
        print("  3. Implement missing Letter pipelines (B, E, F, I, J)")
        return
    ...
    print("\n🏆 SHARD-6 county bootstrap complete!")
    print("\nVerify status with:")
    print("  python scripts/verify_shard6_status.py")
