...
def fetch_parcels_for_cono(co_no: int, limit: int = 3000) -> list:
    rows = run_sql(f"""
        SELECT parcel_id, address, city_name
        FROM fl_parcels
        WHERE co_no = {co_no}
...
def upsert_resolution(
    ...
    run_sql(f"""
        INSERT INTO county_co_no_resolution
            (county_slug, co_no, match_pct, sample_n, is_confirmed, method, notes, updated_at)
        VALUES
...
def candidate_conos(slug: str) -> list[int]:
    ...
    primary = FL_DOR_CONO.get(cs)
    if primary:
        candidates = [primary]
        # Add adjacent values in case registry is wrong (Marion lesson)
        for delta in (-1, 1, -2, 2, 3, -3):
            n = primary + delta
            if 1 <= n <= 67 and n not in candidates:
                candidates.append(n)
        return candidates[:5]
    # Unknown slug — try sequential
    return list(range(1, 68))
...
NEAR_MISS = [
    ...
    ("broward",   6,  "84.0% at n=50 — condo/unit address variance suspected"),
    ("gadsden",  20,  "69.6% at n=23 — real gap below threshold"),
    ("orange",   48,  "54.6% at n=59 — street-name collision, try city/zip disambiguation"),
    ("clay",     10,  "80.0% at n=4 — sample too small"),
    ("franklin", 19,  "81.8% at n=9 — sample too small"),
    ("hardee",   25,  "75.0% at n=4 — only 4 rows with property_address in total"),
    ("lafayette", 34, "100% but n=1 only — single match not evidence"),
]
