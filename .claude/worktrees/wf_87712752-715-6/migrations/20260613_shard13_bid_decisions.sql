-- SHARD-13 Gold Standard Letter J: Bid Decisions Migration
...
CREATE POLICY IF NOT EXISTS "Enable SHARD-13 counties" ON bid_decisions
...
-- Drop and recreate trigger to ensure fresh state
...
CREATE OR REPLACE VIEW v_bid_decisions_j_metrics_shard13 AS
SELECT 
  mca.county_slug,
...
COMMENT ON VIEW v_bid_decisions_j_metrics_shard13 IS 'J letter metrics for SHARD-13 counties: bid_decisions completeness per Shapira Formula contract';

-- SHARD-13 specific verification function for ULTRALOOP protocol
CREATE OR REPLACE FUNCTION shard13_j_verification(county_name TEXT)
RETURNS TABLE(
  county TEXT,
...
BEGIN
  ...
  FROM v_bid_decisions_j_metrics_shard13 v
  WHERE v.county_slug = county_name;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION shard13_j_verification IS 'ULTRALOOP verification function for SHARD-13 J letter status with evidence';
...
-- Create table for migration log if it doesn't exist
...
VALUES (
  '20260613_shard13_bid_decisions',
  NOW(),
  'SHARD-13 bid_decisions table setup for J letter (orange, flagler, santa_rosa, gulf) - Shapira Formula pipeline'
