#!/usr/bin/env python3
"""
SHARD-7 Status Check: highlands, volusia, miami_dade, columbia, madison
"""
import os
import sys
import json
...
def get_current_gold_standard_status():
    ...
    assigned_counties = ['highlands', 'volusia', 'miami_dade', 'columbia', 'madison']
...
if __name__ == "__main__":
    ...
    print("\n=== SUMMARY ===")
    for county, result in evaluations.items():
        if result:
            passed = sum(1 for r in result if r.get('pass'))
            total = len(result)
            print(f"{county}: {passed}/{total} letters passing")