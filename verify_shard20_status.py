#!/usr/bin/env python3
"""
SHARD-20 VERIFICATION PROTOCOL
...
def set_statement_timeout():
    ...
    try:
        # Extended timeout for our HTTP client
        global client
...
def identify_priority_work(evaluations: Dict) -> List[Dict]:
    """Identify priority work based on current evaluations and briefing priorities"""
    priorities = []
    
    # From briefing: B, I, J are critical failing letters across all counties
    # Priority order based on leverage and briefing guidance
    ...
    for county in TARGET_COUNTIES:
        ...
        if evaluation.get('letters'):
            ...
            # Critical letters from briefing analysis
            if letters.get('grade_b') == 'FAIL':
                county_priorities.append(('B', 'verified_outcomes', 'Critical - Independent source needed'))
                priority_score += 5
            
            if letters.get('grade_i') == 'FAIL':
                county_priorities.append(('I', 'property_cards', 'Critical - Property card completeness'))
                priority_score += 5
                
            if letters.get('grade_j') == 'FAIL':
                county_priorities.append(('J', 'deal_thesis', 'Critical - Shapira deal analysis'))
                priority_score += 5
            
            # High value letters
            if letters.get('grade_c') == 'FAIL':
                county_priorities.append(('C', 'parity_clean', 'Parity clean rate <95%'))
                priority_score += 3
                
            if letters.get('grade_d') == 'FAIL':
                county_priorities.append(('D', 'parity_any', 'Parity any rate <95%'))
                priority_score += 3
                
            if letters.get('grade_e') == 'FAIL':
                county_priorities.append(('E', 'parcel_linkage', 'Parcel linkage <95%'))
                priority_score += 3
                
            if letters.get('grade_f') == 'FAIL':
                county_priorities.append(('F', 'tier1_sold', 'Tier1 sold amount <95%'))
                priority_score += 2
                
            if letters.get('grade_g') == 'FAIL':
                county_priorities.append(('G', 'zoning_kpi', 'Zoning density/FAR/parking'))
                priority_score += 2
        
        elif evaluation.get('basic_metrics'):
            # Fallback priority based on basic metrics
            metrics = evaluation['basic_metrics']
            
            if metrics.get('verification_pct', 0) < 95:
                county_priorities.append(('B', 'verified_outcomes', 'Verification rate below 95%'))
                priority_score += 5
            
            if metrics.get('parcel_linkage_pct', 0) < 95:
                county_priorities.append(('E', 'parcel_linkage', 'Parcel linkage below 95%'))
                priority_score += 3
        
        priorities.append({
            'county': county,
            'failing_criteria': county_priorities,
            'priority_score': priority_score,
            'total_failing': len(county_priorities)
        })
...
def main():
    ...
    logger.info("🔍 SHARD-20 VERIFICATION PROTOCOL EXECUTION")
    ...
    try:
        ...
        for i, priority in enumerate(priorities[:3], 1):
            ...
            failing = priority['failing_criteria']
            
            logger.info(f"{i}. {county.upper()} (score: {score}, failing: {len(failing)})")
            for letter, area, reason in failing[:3]:  # Top 3 per county
                logger.info(f"   Letter {letter}: {area} - {reason}")
